...
app.factory('dataservice', function ($http) {
    ...
    return {
        getCountExpense: function (callback) {
            $http.post('/Admin/FundWarning/CountExpense').then(callback);
        },
        getCountReceipt: function (callback) {
            $http.post('/Admin/FundWarning/CountReceipt').then(callback);
        },
    }
});
...
app.config(function ($routeProvider, $validatorProvider, $translateProvider) {
    $translateProvider.useUrlLoader('/Admin/FundWarning/Translation');
...
app.controller('index', function ($scope, $rootScope, $confirm, $compile, $uibModal, DTOptionsBuilder, DTColumnBuilder, DTInstances, dataservice, $filter, $translate) {
    ...
    dataservice.getCountExpense(function (rs) {rs=rs.data;
        $rootScope.countExpense = rs;
    });
    dataservice.getCountReceipt(function (rs) {rs=rs.data;
...
app.controller('fundEntryExpense', function ($scope, $rootScope, $confirm, $compile, $uibModal, DTOptionsBuilder, DTColumnBuilder, DTInstances, dataservice, $filter, $translate) {
    ...
    vm.dtColumns.push(DTColumnBuilder.newColumn('catName').withTitle('{{"FW_LIST_COL_CAT_NAME" | translate}}').renderWith(function (data, type) {
        return data;
    }));

    vm.dtColumns.push(DTColumnBuilder.newColumn('aetType').withTitle('{{"FW_LIST_COL_AET_TYPE" | translate}}').renderWith(function (data, type) {
        if (data == "Receipt") {
            return "Thu";
        }
        else {
    ...
    vm.dtColumns.push(DTColumnBuilder.newColumn('maxTotal').withTitle('{{"FW_LIST_COL_MAX_TOTAL" | translate}}').renderWith(function (data, type) {
        return '<span class="text-danger">' + $filter('currency')(data, '', 0) + '</span>';
    }));
    vm.dtColumns.push(DTColumnBuilder.newColumn('total').withTitle('{{"FW_LIST_COL_TOTAL" | translate}}').renderWith(function (data, type) {
        return '<span class="text-danger">' + $filter('currency')(data, '', 0) + '</span>';
    }));
    vm.dtColumns.push(DTColumnBuilder.newColumn('currency').withTitle('{{"FW_LIST_COL_CURRENCY" | translate}}').renderWith(function (data, type) {
        return data;
    }));

    vm.dtColumns.push(DTColumnBuilder.newColumn('fromDate').withTitle('{{"FW_LIST_COL_FROM_DATE" | translate}}').renderWith(function (data, type) {
        return data != "" ? $filter('date')(new Date(data), 'dd/MM/yyyy') : null;
    }));
    vm.dtColumns.push(DTColumnBuilder.newColumn('toDate').withTitle('{{"FW_LIST_COL_TO_DATE" | translate}}').renderWith(function (data, type) {
        return data != "" ? $filter('date')(new Date(data), 'dd/MM/yyyy') : null;
    }));
