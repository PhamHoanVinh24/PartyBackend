...
app.factory('dataservice', function ($http) {
    ...
    return {
        ...
        getListWareHouse: function (callback) {
            $http.get('/Admin/EDMSQRCodeManager/GetListWareHouse').then(callback);
        },
        getListFloorWithListWareHouse: function (data, callback) {
            $http.get('/Admin/EDMSQRCodeManager/GetListFloorWithListWareHouse?listWareHouse=' + data).then(callback);
        },
        getListLineWithListFloor: function (data, callback) {
            $http.get('/Admin/EDMSQRCodeManager/GetListLineWithListFloor?listFloor=' + data).then(callback);
        },
        getListRackWithListLine: function (data, callback) {
            $http.get('/Admin/EDMSQRCodeManager/GetListRackWithListLine?listLine=' + data).then(callback);
        },
        getListRackPositionWithListRack: function (data, callback) {
            $http.get('/Admin/EDMSQRCodeManager/GetListRackPositionWithListRack?listRack=' + data).then(callback);
        },
        ...
        getListCabinet: function (callback) {
            $http.post('/Admin/EDMSQRCodeManager/GetListCabinet').then(callback);
        },
        searchBox: function (data, callback) {
            $http.post('/Admin/EDMSQRCodeManager/searchBox', data).then(callback);
        }
    };
});
...
app.controller('index', function ($scope, $rootScope, $compile, $uibModal, DTOptionsBuilder, DTColumnBuilder, DTInstances, dataservice, $location, $filter) {
    ...
    $scope.searchBox = function () {
        if ($scope.searchBoxModel.FromDate !== '' || $scope.searchBoxModel.ToDate !== '' || $scope.searchBoxModel.CabinetCode !== '') {
            dataservice.searchBox($scope.searchBoxModel, function (rs) {
                rs = rs.data;
                if (rs.Error) {
                    ...
                    if (rs.Object.length > 0) {
                        ...
                        App.toastrSuccess(caption.EDMS_QRCODE_MANAGER_SEARCH_SUCCESS + rs.Object.length + caption.EDMS_QRCODE_MANAGER_FOUND_BOX + countResult + ' ' + caption.EDMS_QRCODE_MANAGER_BOX_SECCESS);
                    } else {
                        App.toastrError(caption.EDMS_QRCODE_MANAGER_NOT_FOUND_BOX);
                    }
                }
            });
        } else {
            App.toastrError(caption.EDMS_QRCODE_MANAGER_ENTER);
        }
    };
    $scope.isSearch = false;
    $scope.showSearch = function () {
