...
app.factory('dataserviceImprovement', function ($http) {
    ...
    return {
        ...
        getUserTransfer: function (data, callback) {
            $http.post('/Admin/AssetImprovement/GetUserTransfer?DepartmentCode=' + data).then(callback);
        },
        ...
        getAssetInTicket: function (data, callback) {
            $http.post('/Admin/AssetImprovement/GetAssetInTicket?ticketCode=' + data).then(callback);
        },
        ...
        getStepWorkFlow: function (data, callback) {
            $http.post('/Admin/AssetImprovement/GetStepWorkFlow?code=' + data).then(callback);
        },
        ...
        getItemTemp: function (data, callback) {
            $http.post('/Admin/AssetImprovement/GetItemTemp?id=' + data).then(callback)
        },
        getListRepeat: function (data, callback) {
            $http.post('/Admin/AssetImprovement/GetListRepeat?code=' + data).then(callback);
        },
        getActionStatus: function (data, callback) {
            $http.get('/Admin/AssetImprovement/GetActionStatus?code=' + data).then(callback);
        },

        getLogStatus: function (data, callback) {
            $http.post('/Admin/AssetImprovement/GetLogStatus?code=' + data).then(callback);
        },
...
app.controller('Ctrl_ESEIM_IMPROVEMENT', function ($scope, $rootScope, $compile, dataserviceImprovement, $filter, $cookies, $translate) {
    ...
    $rootScope.ObjectTypeFile = "ASSET_IMPROVE";
...
app.controller('index', function ($scope, $rootScope, $confirm, $compile, $uibModal, $location) {
    $scope.modelsearch = {
        TicketCode: "",
        Title: "",
        FromDate: "",
...
app.controller('tableheader', function ($scope, $rootScope, $confirm, $compile, $uibModal, DTOptionsBuilder, DTColumnBuilder, DTInstances, dataserviceImprovement, $filter, myService, $location) {
    var vm = $scope;
    $scope.selected = [];
    ...
    vm.dtColumns.push(DTColumnBuilder.newColumn('Status').withTitle('{{"ASSETIMPRO_TAB_HEADER_COL_STATUS" | translate}}').renderWith(function (data, type, full) {
...
app.controller('add', function ($scope, $rootScope, $compile, $uibModal, dataserviceImprovement, $location) {
    $scope.CheckCode = '';
    $scope.model = {
        Status: '',
        //TicketID: ''
    ...
    $scope.initLoad = function () {
        ...
        dataserviceImprovement.getWorkFlow(function (result) {
            result = result.data;
            $rootScope.lstWorkflow = result;
        });
        dataserviceImprovement.getStatusAct(function (rs) {
            rs = rs.data;
            $scope.lstStatus = rs;
        })
        dataserviceImprovement.suggestWF($scope.modelWf.ObjectType, function (rs) {
            rs = rs.data;
        ...
        if ($scope.addform.validate() && !validationSelect($scope.model).Status) {
            ...
            dataserviceImprovement.insertHeader($scope.model, function (result) {
                ...
                if (result.Error) {
                    ...
                    dataserviceImprovement.createWfInstance($scope.modelWf, function (rs) {
                        rs = rs.data;
                        if (rs.Error) {
                            App.toastrError(rs.Title);
                        } else {
    ...
    $scope.changeWorkFlowInts = function (wfCatCode) {
        dataserviceImprovement.getStepWorkFlow(wfCatCode, function (rs) {
            data = rs.data;
    ...
    $scope.changleSelect = function (SelectType) {
        ...
        if (SelectType == "DepartTransfer" && $scope.model.DepartTransfer != "") {
            ...
            dataserviceImprovement.getUserTransfer($scope.model.DepartTransfer, function (rs) {
                rs = rs.data;
...
app.controller('editImprovement', function ($scope, $rootScope, $compile, $uibModal, dataserviceImprovement, $filter, $location, myService) {
    ...
    $scope.initData = function () {
        dataserviceImprovement.getItemImprovement(para, function (rs) {
            ...
            dataserviceImprovement.getAssetInTicket($rootScope.TicketCode, function (rs) {
                rs = rs.data;
                $scope.lstProd = rs;
            })
    ...
    if (para == undefined || para <= 0) {
        location.href = "/Admin/AssetImprovement";
    }
    ...
    $scope.editInstAct = function (id, objCode) {
        dataserviceImprovement.checkPermissionEditActivityById(id, function (rs) {
            rs = rs.data;
            if (!rs) {
                return App.toastrError(caption.COM_MSG_PERMISSION_EDIT_ACT);
            }
            dataserviceImprovement.getItemActInst(id, function (rs) {
                ...
                modalInstance.result.then(function (d) {
                    $rootScope.loadDiagramWfInst($scope.model.TicketCode, $scope.modelWf.ObjectType);
                    dataserviceImprovement.getLogStatus($scope.model.TicketCode, function (rs) {
                        rs = rs.data;
    ...
    $scope.changeStatus = function () {
        if ($scope.model.Status == "REPEAT_REQUIRE_REWORK" || $scope.model.Status == "FINAL_REQUIRE_REWORK") {
            dataserviceImprovement.getListRepeat($scope.model.TicketCode, function (rs) {
                rs = rs.data;
                var arr = [];
            ...
            dataserviceImprovement.updateHeaderImprovement($scope.model, function (rs) {
                ...
                if (rs.Error) {
                    ...
                    if ($scope.model.Status != "FINAL_DONE") {
                        dataserviceImprovement.getActionStatus($scope.model.TicketCode, function (rs) {
                            rs = rs.data;
                        ...
                        dataserviceImprovement.getItemTemp(para, function (rs) {
                            rs = rs.data;
                            var data = rs.Object;
                            //Item header
    ...
    $scope.print = function () {
        ...
        var content = '<p><strong>Đơn vị:</strong>................................. </p>' +
            ...
            '<p></p>' +
            '<p style="text-align: center;"><strong>PHIẾU BẢO DƯỠNG TÀI SẢN</strong></p>' +
            ...
            '<p>Đơn vị bảo dưỡng: <strong>' + unit + '</strong></p>' +
...
app.controller('fileAsset', function ($scope, $rootScope, $compile, $uibModal, DTOptionsBuilder, DTColumnBuilder, DTInstances, dataserviceImprovement, $filter, dataserviceSupplier) {
    var vm = $scope;
    $scope.selected = [];
    $scope.selectAll = false;
    $scope.toggleAll = toggleAll;
    ...
    $scope.extend = function (id) {
        ...
        dataserviceSupplier.getDefaultRepo($rootScope.AssetCode, 'ASSET_IMPROVE', function (rs) {
            rs = rs.data;
            var data = rs !== null ? rs : { CatCode: '', ObjectCode: $rootScope.AssetCode, ObjectType: 'ASSET_IMPROVE' };
            var modalInstance = $uibModal.open({
...
app.controller('tabFileHistory', function ($scope, $rootScope, $compile, $uibModal, $uibModalInstance, DTOptionsBuilder, DTColumnBuilder, DTInstances, dataserviceImprovement, $filter, para) {
    ...
    $scope.getObjectFile = function (id, mode) {
        dataserviceImprovement.getItemFile(id, true, mode);
    };
