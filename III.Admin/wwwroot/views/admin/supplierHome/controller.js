var ctxfolder = "/views/admin/supplierHome";
...
app.factory('dataservice', function ($http) {
    ...
    return {
        ...
        getCountSuppHome: function (callback) {
            $http.get('/Admin/Supplier/GetCountSuppHome').then(callback);
        }
    };
});
...
app.config(function ($routeProvider, $validatorProvider, $translateProvider, $qProvider) {
    $translateProvider.useUrlLoader('/Admin/UserBusyOrFree/TranslationSuppHome');
...
app.controller('index', function ($scope, $rootScope, $sce, $compile, $uibModal, DTOptionsBuilder, DTColumnBuilder, DTInstances, dataservice, $filter, $translate) {
    ...
    $scope.initBoxCard = function () {
        dataservice.getCountSuppHome(function (rs) {
            rs = rs.data;
            $scope.all = rs.All;
            $scope.pending = rs.Pending;
        })
        $scope.switchOn = true;
        dataservice.getActionSupplier(function (rs) {
            rs = rs.data;
            $scope.lstSup = rs;
        });
        window.tabler = {
        ...
        dataservice.getCountSupplier(function (rs) {
            rs = rs.data;
        ...
        dataservice.amchartSupplier(function (rs) {
            rs = rs.data;
            monthsup = [];
            sumsup = ['sum'];
            activesup = ['active'];
            ...
            for (var i = 0; i < rs.length; i++) {
                ...
                monthsup.push(caption.COM_LBL_MONTH + ' ' + (rs[i].Month));
            }
            setTimeout(function () {
                var chart = c3.generate({
                    ...
                    data: {
                        ...
                        names: {
                            // name of each serie
                            'sum': caption.SHC_LBL_TOTAL_NUMBER,
                            'active': caption.SHC_LBL_BEING_ACTIVE,
                            'deactive': caption.SHC_LBL_SHUT_DOWN,
                        }
                    },
                    axis: {
                        x: {
    ...
    $scope.switchDiv = function () {
        ...
        divMenuDigital = $('#menuSupplierHome');
        divChart = $('#chartSupplierHome');
...
app.controller('index1', function ($scope, $rootScope, $sce, $compile, $uibModal, DTOptionsBuilder, DTColumnBuilder, DTInstances, dataservice, $filter, $translate) {

    dataservice.getActionCustomer(function (rs) {
        rs = rs.data;
        $scope.lstCus = rs;

    });
    dataservice.getCountCustomer(function (rs) {
        rs = rs.data;
    ...
    dataservice.amchartCustomer(function (rs) {
        rs = rs.data;
        monthcus = [];
        sumcus = ['sum'];
        activecus = ['active'];
        ...
        for (var i = 0; i < rs.length; i++) {
            ...
            monthcus.push(caption.COM_LBL_MONTH + ' ' + (rs[i].Month));
        }
