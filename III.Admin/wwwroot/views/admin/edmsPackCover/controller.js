...
app.factory('dataservice', function ($http) {
    ...
    return {
        ...
        getListPack: function (callback) {
            $http.post('/Admin/EDMSPackCover/GetListPack').then(callback);
        },
        insertPack: function (data, callback) {
            $http.post('/Admin/EDMSPackCover/InsertPack', data).then(callback);
        },
        getItemPack: function (data, callback) {
            $http.post('/Admin/EDMSPackCover/GetItemPack?id=' + data).then(callback);
        },
        updatePack: function (data, callback) {
            $http.post('/Admin/EDMSPackCover/UpdatePack', data).then(callback);
        },
        deletePack: function (data, callback) {
            $http.post('/Admin/EDMSPackCover/DeletePack?id=' + data).then(callback);
        },
    };
});
...
app.config(function ($routeProvider, $validatorProvider, $httpProvider, $translateProvider) {
    $translateProvider.useUrlLoader('/Admin/EDMSPackCover/Translation');
...
app.controller('index', function ($scope, $rootScope, $filter, $compile, $uibModal, DTOptionsBuilder, DTColumnBuilder, DTInstances, dataservice, $translate, $timeout) {
    var vm = $scope;
    $scope.model = {
        Name: '',
        FromDate: '',
    ...
    $scope.count = 0;
    var today = new Date();
    $scope.model.FromDate = $filter('date')(today, 'dd/MM/yyyy');
    $scope.model.ToDate = $filter('date')(today, 'dd/MM/yyyy');
    ...
    vm.dtOptionsPackCover = DTOptionsBuilder.newOptions()
        .withOption('ajax', {
            url: "/Admin/EDMSPackCover/JtablePack",
