var ctxfoldertwitterSocialConfig = "/views/admin/twitterSocialConfig";
var ctxfoldertwitterSocialConfigMessage = "/views/message-box";
...
app.factory('dataservice', function ($http) {
    ...
    return {
        ...
        getconditon: function (data, callback) {
            $http.post('/Admin/twitterSocialConfig/GetCondition?id=' + data).then(callback);
        },
        getbotsocial: function (data, callback) {
            $http.post('/Admin/twitterSocialConfig/GetBotSocial?bottype=' + data).then(callback);
        },
...
app.config(function ($routeProvider, $validatorProvider, $translateProvider, $qProvider) {
    $translateProvider.useUrlLoader('/Admin/twitterSocialConfig/Translation');
    caption = $translateProvider.translations();
    $qProvider.errorOnUnhandledRejections(false);
    $routeProvider
        .when('/', {
            templateUrl: /*ctxfolderLmsDashBoard + '/index.html'*/ctxfoldertwitterSocialConfig + '/index-course.html',
            controller: 'index'
        })
...
app.controller('index', function ($scope, $rootScope, $sce, $compile, $uibModal, DTOptionsBuilder, DTColumnBuilder, DTInstances, dataservice, $filter, $translate) {
        ...
        if ($scope.model.Id != "") {
            ...
            dataservice.runcrawler($scope.model.Id, function (rs) {
                ...
                login = {
                    ...
                    Description: decodeHTML(rs.Description)
                }
    ...
    $scope.Runpostcontent = function () {
        if ($scope.model.Id != "") {
            ...
            dataservice.runcrawler($scope.model.Id, function (rs) {
                ...
                data = JSON.parse(rs.ConditionStatement)
                postcontent = {
                    "Post tweet": decodeHTML(data.Post.Main_post)
                }
    ...
    $scope.Rununlikepost = function () {
        if ($scope.model.Id != "") {
            ...
            dataservice.runcrawler($scope.model.Id, function (rs) {
                ...
                data = JSON.parse(rs.ConditionStatement)
                unlike = {
                    "Unlikepost": decodeHTML(data.Unlike.Post)
                }
                ws.onopen = function () {
                    //ws.send("Start");
                    ws.send(login)
    ...
    //Unfollow
    $scope.isrununfollow = true;
    $scope.Stopunfollow = function () {
        if (!$scope.isrununfollow) {
            $scope.isrununfollow = true;
        } else {
            $scope.isrununfollow = false;
        }
        dataservice.stopcrawler($scope.model.Id, function (rs) {
            var ws = new WebSocket("ws://localhost:9091");
            rs = rs.data
    ...
    $scope.Rununfollow = function () {
        if ($scope.model.Id != "") {
            ...
            dataservice.runcrawler($scope.model.Id, function (rs) {
                ...
                data = JSON.parse(rs.ConditionStatement)
                unfollow = {
                    "Unfollow": decodeHTML(data.Unfollow)
                }

                ws.onopen = function () {
                    //ws.send("Start");
                    ws.send(login)
                    ws.send(JSON.stringify(unfollow))
...
{ 'id': 'Hang Bui', 'label': 'Hang Bui' }
    ...
    $scope.ConditionStatement = {
        ...
        Post:
        {
            Main_post:
            {
                media: 'https://www.instagram.com/p/Cb6c3U9pqC8/',
                friend: 'comeasyouare257',
            ...
            Comment_post:
            {
                media: [],
                friend: 'comeasyouare257',
                Content: 'that nice, so beautyful \n',
                post: ['https://twitter.com/sugatheartist/status/1485108360106196995', 'https://twitter.com/woniespolcolve/status/1485108847928881163']
            }
        ...
        Like:
        {
            Post:
            {
                linkpost: ['https://twitter.com/sugatheartist/status/1485108360106196995','https://twitter.com/woniespolcolve/status/1485108847928881163'],
                friends: "",
                keywords: [],
                from: "",
                to: ""
        ...
        Follow:
        {
            Name: ['BillGates', 'Google', 'googleanalytics', 'sengineland', 'lalalisaaa__m']
        },
        Unfollow:
        {
            Name: ['BillGates', 'Google', 'googleanalytics', 'sengineland', 'lalalisaaa__m']
    ...
    $scope.addhashtag = function () {
        ...
        var valueKeyword = $scope.model.hashtag1;
        // nếu keyword chưa có 
        if ($scope.ConditionStatement.Post.Main_post.hashtag.findIndex(x => x == valueKeyword) == -1) {
            // 2. thêm keyword vào mảng
            $scope.ConditionStatement.Post.Main_post.hashtag.push(valueKeyword);
        }
        // 3. clear trường keyword
        $scope.model.hashtag1 = "";
    }
    // Unlikecomment
    ...
    dataservice.getbotsocial(4,function (rs) {
        rs = rs.data;
        $scope.listbot = rs;
    });
    $scope.initData();
