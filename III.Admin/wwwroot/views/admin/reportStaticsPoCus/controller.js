var ctxfolder = "/views/admin/reportStaticsPoCus";
...
app.factory('dataservice', function ($http) {
    ...
    return {
        getListContract: function (callback) {
            $http.post('/Admin/reportStaticsPoCus/GetListContract/').then(callback);
        },
        getListCustomer: function (callback) {
            $http.post('/Admin/reportStaticsPoCus/GetListCustomer/').then(callback);
        },
...
app.config(function ($routeProvider, $validatorProvider, $translateProvider) {
    $translateProvider.useUrlLoader('/Admin/reportStaticsPoCus/Translation');
...
app.controller('index', function ($scope, $rootScope, $confirm, $compile, $uibModal, DTOptionsBuilder, DTColumnBuilder, DTInstances, dataservice, $http, $filter) {
    ...
    vm.dtOptions = DTOptionsBuilder.newOptions()
        .withOption('ajax', {
            url: "/Admin/reportStaticsPoCus/Jtable",
    ...
    vm.dtColumns.push(DTColumnBuilder.newColumn('ContractCode').withTitle('{{ "RSPC_LIST_COL_CONTRACT_CODE" | translate }}').notSortable().withOption('sClass', 'tcenter').renderWith(function (data, type) {
        return data;
    }));
    vm.dtColumns.push(DTColumnBuilder.newColumn('ContractNo').withTitle('{{ "RSPC_LIST_COL_CONTRACT_NO" | translate }}').notSortable().renderWith(function (data, type) {
        return data;
    }));
    vm.dtColumns.push(DTColumnBuilder.newColumn('Title').withTitle('{{ "RSPC_LIST_COL_TITLE" | translate }}').notSortable().renderWith(function (data, type) {
        return data;
    }));
    vm.dtColumns.push(DTColumnBuilder.newColumn('EffectiveDate').withTitle('{{ "RSPC_LIST_COL_EFFECTIVE_DATE" | translate }}').notSortable().renderWith(function (data, type) {
        if (data != null && data != '')
            return $filter('date')(new Date(data), 'dd/MM/yyyy');
        else
            return '';
    }));
    vm.dtColumns.push(DTColumnBuilder.newColumn('CusName').withTitle('{{ "RSPC_LIST_COL_CUS_NAME" | translate }}').notSortable().renderWith(function (data, type) {
        return data;
    }));
    vm.dtColumns.push(DTColumnBuilder.newColumn('ProductName').withTitle('{{ "RSPC_LIST_COL_PRODUCT_NAME" | translate }}').notSortable().renderWith(function (data, type) {
        return data;
    }));
    vm.dtColumns.push(DTColumnBuilder.newColumn('ProductTypeName').withTitle('{{"RSPC_LIST_COL_PRODUCT_TYPE_NAME" | translate }}').notSortable().withOption('sClass', 'tcenter').renderWith(function (data, type) {
        return data;
    }));
    vm.dtColumns.push(DTColumnBuilder.newColumn('RevenueAfterTaxVnd').withTitle('{{ "RSPC_LIST_COL_REVENUE_AFTER_TAX" | translate }}').notSortable().withOption('sClass', 'tcenter').renderWith(function (data, type) {
    ...
    $scope.export = function () {
        ...
        location.href = "/Admin/reportStaticsPoCus/ExportExcel?"
            + "page=" + page
            + "&row=" + length
