var ctxfolderYoutubeSocialConfig = "/views/admin/youtubeSocialConfig";
var ctxfolderYoutubeSocialConfigMessage = "/views/message-box";
...
app.factory('dataservice', function ($http) {
    ...
    return {
        ...
        getconditon: function (data, callback) {
            $http.post('/Admin/youtubeSocialConfig/GetCondition?id=' + data).then(callback);
        },
        getbotsocial: function (data, callback) {
            $http.post('/Admin/youtubeSocialConfig/GetBotSocial?bottype=' + data).then(callback);
        },
        runcrawler: function (data, callback) {
            $http.post('/Admin/YoutubeSocial/RunCrawler?id=' + data).then(callback);
        },
        stopcrawler: function (data, callback) {
            $http.post('/Admin/YoutubeSocial/StopCrawler?id=' + data).then(callback);
        },
    };
});
...
app.config(function ($routeProvider, $validatorProvider, $translateProvider, $qProvider) {
    $translateProvider.useUrlLoader('/Admin/youtubeSocialConfig/Translation');
    caption = $translateProvider.translations();
    $qProvider.errorOnUnhandledRejections(false);
    $routeProvider
        .when('/', {
            templateUrl: /*ctxfolderLmsDashBoard + '/index.html'*/ctxfolderYoutubeSocialConfig + '/index-course.html',
            controller: 'index'
        })
    ...
    $scope.Runcommentpost = function () {
        if ($scope.model.Id != "") {
            ...
            dataservice.runcrawler($scope.model.Id, function (rs) {
                ...
                postcomment = {
                    "Post Comment": decodeHTML(data.Post.Comment_post)
                }
    ...
    $scope.Runpostcontent = function () {
        if ($scope.model.Id != "") {
            ...
            dataservice.runcrawler($scope.model.Id, function (rs) {
                ...
                data = JSON.parse(rs.ConditionStatement)
                postcontent = {
                    "Post video": decodeHTML(data.Post.Main_post)
                }
    ...
    $scope.Rununlikepost = function () {
        if ($scope.model.Id != "") {
            ...
            dataservice.runcrawler($scope.model.Id, function (rs) {
                ...
                data = JSON.parse(rs.ConditionStatement)
                unlike = {
                    Unlike_post: decodeHTML(data.Unlike.Post)
                }
                ws.onopen = function () {
                    //ws.send("Start");
                    ws.send(login)
    ...
    $scope.Runfollow = function () {
        if ($scope.model.Id != "") {
            ...
            dataservice.runcrawler($scope.model.Id, function (rs) {
                ...
                data = JSON.parse(rs.ConditionStatement)
                subcriber = {
                    "Subcriber": decodeHTML(data.Subcriber)
                }
                ws.onopen = function () {
                    //ws.send("Start");
                    ws.send(login)
                    ws.send(JSON.stringify(subcriber))
...
{ 'id': 'Hang Bui', 'label': 'Hang Bui' }
    ...
    $scope.ConditionStatement = {
        ...
        Post:
        {
            Main_post:
            {
                ...
                content: "new video for new wwork"
            },
            Comment_post:
            {
                link: ["https://www.youtube.com/watch?v=UVbv-PJXm14", "https://www.youtube.com/watch?v=RzE7ZcGwFvU"],
                content: "hello that video nice"
        ...
        Like:
        {
            Post:
            {
                link: [{ "id": "CK6K1EjsJAk" } ,{ "id": "RzE7ZcGwFvU" }],
                keywords: [],
                from: "",
                to: ""
            },
        ...
        Subcriber:
        {
            linkSub: ["https://www.youtube.com/watch?v=UVbv-PJXm14", "https://www.youtube.com/watch?v=RzE7ZcGwFvU"]
            /*$scope.listsearch*/
        },
        Unsubcriber:
        {
            linkunSub: ["https://www.youtube.com/watch?v=UVbv-PJXm14", "https://www.youtube.com/watch?v=RzE7ZcGwFvU"]
        },
        Sharepost: {
            post: [],
            group: [],
    ...
    $scope.initData = function () {
        ...
        $scope.getlistchannel()
        //$scope.getlistplaylist()
        //$scope.getlistvideo()

    };
    
    $scope.getlistchannel = function () {
        var xmlHttp = new XMLHttpRequest();
        xmlHttp.open("GET", "https://youtube.googleapis.com/youtube/v3/subscriptions?part=snippet&channelId=UCcc9UR8iWTp_s6HBEtJNq2Q&key=AIzaSyAh5icqZb-5n7NTLdi9TWvyVcG9U86bJeg", false); // false for synchronous request
        xmlHttp.send(null);
        a = xmlHttp.responseText;
        i = 0
        for (; a[i];) {
    ...
    $scope.getlistplaylist = function () {
        var xmlHttp = new XMLHttpRequest();
        xmlHttp.open("GET", "https://youtube.googleapis.com/youtube/v3/playlists?part=snippet&channelId=UC3B-TX9_7IkIfXpaI5yimdg&key=AIzaSyAh5icqZb-5n7NTLdi9TWvyVcG9U86bJeg", false); // false for synchronous request
        xmlHttp.send(null);
        playlist = xmlHttp.responseText;
        playlist = JSON.parse(playlist)
        data = {
            "id": playlist.items[0].id,
            "label": playlist.items[0].id,
        }
        $scope.listgroup = data
    };
    $scope.getlistvideo = function () {
        var xmlHttp = new XMLHttpRequest();
        xmlHttp.open("GET", "https://youtube.googleapis.com/youtube/v3/playlistItems?part=snippet&playlistId=PLDgLZ-JxFtT_HFstj0QQcLxy5BpFkqJ3d&key=AIzaSyAh5icqZb-5n7NTLdi9TWvyVcG9U86bJeg", false); // false for synchronous request
        xmlHttp.send(null);
        listvideo = xmlHttp.responseText;
        i = 0
        for (; listvideo[i];) {
            listvideo = decodeHTML(listvideo);
            i++;
        }
    ...
    $scope.getlistsearch = function () {
        var xmlHttp = new XMLHttpRequest();
        xmlHttp.open("GET", "https://youtube.googleapis.com/youtube/v3/search?part=snippet&maxResults=20&q=Nh%E1%BA%A1c%20tr%E1%BA%BB&key=AIzaSyAh5icqZb-5n7NTLdi9TWvyVcG9U86bJeg", false); // false for synchronous request
        xmlHttp.send(null);
        listsearch = xmlHttp.responseText;
        i = 0
        for (; listsearch[i];) {
            listsearch = decodeHTML(listsearch);
            i++;
        }
        listsearch = JSON.parse(listsearch)
        $scope.listgroup = listsearch.data
    };
    dataservice.getbotsocial(6,function (rs) {
        rs = rs.data;
        $scope.listbot = rs;
    });
    $scope.initData();
