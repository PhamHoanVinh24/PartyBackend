...
app.factory('dataserviceLiquidation', function ($http) {
    ...
    return {
        ...
        getAssetInTicket: function (data, callback) {
            $http.post('/Admin/AssetLiquidation/GetAssetInTicket?ticketCode=' + data).then(callback);
        },
        ...
        getStepWorkFlow: function (data, callback) {
            $http.post('/Admin/AssetLiquidation/GetStepWorkFlow?code=' + data).then(callback);
        },
        ...
        getItemTemp: function (data, callback) {
            $http.post('/Admin/AssetLiquidation/GetItemTemp?id=' + data).then(callback)
        },
        getListRepeat: function (data, callback) {
            $http.post('/Admin/AssetLiquidation/GetListRepeat?code=' + data).then(callback);
        },
        getActionStatus: function (data, callback) {
            $http.get('/Admin/AssetLiquidation/GetActionStatus?code=' + data).then(callback);
        },
        getLogStatus: function (data, callback) {
            $http.post('/Admin/AssetLiquidation/GetLogStatus?code=' + data).then(callback);
        },
...
app.controller('Ctrl_ESEIM_LIQUIDATION', function ($scope, $rootScope, $compile, $uibModal, dataserviceLiquidation, $cookies, $translate) {
    ...
    $rootScope.ObjectTypeFile = "ASSET_LIQUIDATION";
...
app.controller('index', function ($scope, $rootScope, $uibModal, dataserviceLiquidation, $location) {
    $scope.modelsearch = {
        Title: '',
        TicketCode: '',
        BranchLiquidation: '',
...
app.controller('tabAssetLiquidationTicket', function ($scope, $rootScope, $compile, $uibModal, DTOptionsBuilder, DTColumnBuilder, DTInstances, dataserviceLiquidation, $filter, myService, $location) {
    var vm = $scope;
    $scope.selected = [];
    ...
    vm.dtColumns.push(DTColumnBuilder.newColumn('Status').withTitle('{{"ASLQDT_COL_STATUS" | translate}}').renderWith(function (data, type, full) {
        var status = "";
...
app.controller('tabAssetLiquidation', function ($scope, $rootScope, $compile, $uibModal, DTOptionsBuilder, DTColumnBuilder, DTInstances, dataserviceLiquidation, $filter) {
    ...
    vm.dtColumns.push(DTColumnBuilder.newColumn('action').withTitle('{{"ASLQDT_COL_ACTION" | translate}}').renderWith(function (data, type, full) {
        return '<a title="{{&quot; COM_BTN_DELETE &quot; | translate}}" ng-click="delete(' + full.ID + ')"  class="fs25"><i class="fas fa-trash"style="--fa-primary-color: red;"></i></button>';
    }));
    vm.reloadData = reloadData;
    vm.dtInstance = {};
    function reloadData(resetPaging) {
...
app.controller('add', function ($scope, $rootScope, $uibModal, dataserviceLiquidation, $location) {
    $scope.CheckCode = '';
    ...
    $scope.initLoad = function () {
        ...
        dataserviceLiquidation.getWorkFlow(function (result) {
            result = result.data;
            $rootScope.lstWorkflow = result;
        });
        dataserviceLiquidation.getStatusAct(function (rs) {
            rs = rs.data;
            $scope.lstStatus = rs;
        })

        dataserviceLiquidation.suggestWF($scope.modelWf.ObjectType, function (rs) {
            rs = rs.data;
        ...
        if ($scope.addform.validate() && !validationSelect($scope.model).Status) {
            ...
            dataserviceLiquidation.insertTicket($scope.model, function (result) {
                ...
                if (result.Error) {
                    ...
                    dataserviceLiquidation.createWfInstance($scope.modelWf, function (rs) {
                        rs = rs.data;
                        if (rs.Error) {
                            App.toastrError(rs.Title);
                        } else {
    ...
    $scope.changeWorkFlowInts = function (wfCatCode) {
        dataserviceLiquidation.getStepWorkFlow(wfCatCode, function (rs) {
            data = rs.data;
...
app.controller('editLiquidation', function ($scope, $rootScope, $uibModal, dataserviceLiquidation, $filter, myService, $location) {
    $scope.isDisabled = true;
    ...
    $scope.initData = function () {
        dataserviceLiquidation.getItemLiquidation(para, function (rs) {
            ...
            dataserviceLiquidation.getAssetInTicket($rootScope.TicketCode, function (rs) {
                rs = rs.data;
                $scope.lstProd = rs;
            })
    ...
    if (para == undefined || para <= 0) {
        location.href = "/Admin/AssetLiquidation";
    }
    ...
    $scope.changeStatus = function () {
        if ($scope.model.Status == "REPEAT_REQUIRE_REWORK" || $scope.model.Status == "FINAL_REQUIRE_REWORK") {
            dataserviceLiquidation.getListRepeat($scope.model.TicketCode, function (rs) {
                rs = rs.data;
                var arr = [];
            ...
            dataserviceLiquidation.updateLiquidation($scope.model, function (result) {
                ...
                if (result.Error) {
                    ...
                    if ($scope.model.Status != "FINAL_DONE") {
                        dataserviceLiquidation.getActionStatus($scope.model.TicketCode, function (rs) {
                            rs = rs.data;
                        ...
                        dataserviceLiquidation.getItemTemp(para, function (rs) {
                            rs = rs.data;
                            var data = rs.Object;

                            //Item header
                            $scope.model = data.data;
                            $scope.model.sLiquidationTime = $filter('date')(new Date($scope.model.LiquidationTime), 'dd/MM/yyyy');
                            //Workflow
    ...
    $scope.editInstAct = function (id, objCode) {
        dataserviceLiquidation.checkPermissionEditActivityById(id, function (rs) {
            rs = rs.data;
            if (!rs) {
                return App.toastrError(caption.COM_MSG_PERMISSION_EDIT_ACT);
            }
            dataserviceLiquidation.getItemActInst(id, function (rs) {
                ...
                modalInstance.result.then(function (d) {
                    $rootScope.loadDiagramWfInst($scope.model.TicketCode, $scope.modelWf.ObjectType);
                    dataserviceLiquidation.getLogStatus($scope.model.TicketCode, function (rs) {
                        rs = rs.data;
    ...
    $scope.print = function () {
        ...
        var infoTime = $scope.model.sLiquidationTime.split("/");

        var personRecei = "";
        for (var i = 0; i < $scope.model.listEmp.length; i++) {
            if ($scope.model.UserLiquidation === $scope.model.listEmp[i].Code) {
                personRecei = $scope.model.listEmp[i].Name;
                break;
            }
        }
        ...
        var content = '<p><strong>Đơn vị:</strong>................................. </p>' +
            ...
            '<p></p>' +
            '<p style="text-align: center;"><strong>PHIẾU THANH LÝ TÀI SẢN</strong></p>' +
            ...
            '<p>Người thanh lý: <strong>' + personRecei + '</strong></p>' +
            '<p>Người mua: <strong>' + $scope.model.UserBuy + '</strong></p>' +
            '<p>Địa điểm: ' + $scope.model.LocationLiquidation + '</p>' +
            '<p>Ghi chú: ' + $scope.model.Description + '</p>' +
...
app.controller('assetDetail', function ($scope, $rootScope, $compile, $uibModal, DTOptionsBuilder, DTColumnBuilder, DTInstances, dataserviceLiquidation, $filter) {
    ...
    vm.dtColumns.push(DTColumnBuilder.newColumn('CurrencyAsset').withTitle('{{"ASLQDT_CURD_CURRENCY" | translate}}').renderWith(function (data, type) {
        return data;
    }));
    vm.dtColumns.push(DTColumnBuilder.newColumn('Status').withTitle('{{"ASLQDT_COL_STATUS" | translate}}').renderWith(function (data, type) {
        return data;
...
app.controller('fileAsset', function ($scope, $rootScope, $compile, $uibModal, DTOptionsBuilder, DTColumnBuilder, DTInstances, dataserviceLiquidation, $filter, dataserviceSupplier) {
    var vm = $scope;
    $scope.selected = [];
    $scope.selectAll = false;
    $scope.toggleAll = toggleAll;
    ...
    $scope.extend = function (id) {
        ...
        dataserviceSupplier.getDefaultRepo($rootScope.AssetCode, 'ASSET_LIQUIDATION', function (rs) {
            rs = rs.data;
            var data = rs !== null ? rs : { CatCode: '', ObjectCode: $rootScope.AssetCode, ObjectType: 'ASSET_LIQUIDATION' };
            var modalInstance = $uibModal.open({
...
app.controller('tabFileHistory', function ($scope, $rootScope, $compile, $uibModal, $uibModalInstance, DTOptionsBuilder, DTColumnBuilder, DTInstances, dataserviceLiquidation, $filter, para) {
    ...
    $scope.getObjectFile = function (id, mode) {
        dataserviceLiquidation.getItemFile(id, true, mode);
    };
