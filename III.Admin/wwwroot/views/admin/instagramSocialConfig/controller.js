var ctxfolder = "/views/admin/instagramSocialConfig";
...
{ 'id': 'Hang Bui', 'label': 'Hang Bui' }
    ...
    $scope.ConditionStatement = {
        ...
        Post:
        {
            Main_post:
            {              
                media: ["https://cms.globalacademicresearch.com/IAH_AU/data/image_upload/1478263587maxresdefault-min.jpg", "https://i.ytimg.com/vi/GbdTvxuZsCw/maxresdefault.jpg"],
                friends: "",
                content:""               
            },
            Comment_post:
            {
                linkpost: ["p/CY6-wWsv5mM/", "p/CYmVnslv_WU/", "p/CYPZMDbPk2q", "p/CXy5hp6FdqC/"],
                friends: [],
                content: "",
                keywords:[]
        ...
        Like:
        {
            Post:
            {
                postId: ["p/CXy5hp6FdqC/", "p/CYPZMDbPk2q/", "p/CY4R1oiOwVF/", "p/CY95aP9PzoP/"]
            },

            Comment:
            {
                post: ["p/CXy5hp6FdqC/", "p/CYPZMDbPk2q/", "p/CY4R1oiOwVF/", "p/CY95aP9PzoP/"]
            }
        },
        Unlike:
        {
        ...
        Follow:
        {
            userid: ["kendalljenner", "haileybieber", "jlo", "nickiminaj"]
        },
        Unfollow:
        {
            userid: ["kendalljenner", "haileybieber", "jlo", "nickiminaj"]
    ...
    // add hash tag get content
    $scope.addKeyword = function () {
        // 1. kiểm tra keyword có hay chưa
        var valueKeyword = $scope.model.keywords;
        // nếu keyword chưa có 
        if ($scope.ConditionStatement.Get.Post.hashtag.findIndex(x => x == valueKeyword) == -1) {
            // 2. thêm keyword vào mảng
            $scope.ConditionStatement.Get.Post.hashtag.push(valueKeyword);
        }
    ...
    $scope.addKeyword = function () {
        ...
        var valueKeyword = $scope.model.keywords;
        // nếu keyword chưa có 
        if ($scope.ConditionStatement.Comment_post.friends.findIndex(x => x == valueKeyword) == -1) {
            // 2. thêm keyword vào mảng
            $scope.ConditionStatement.Comment_post.friends.push(valueKeyword);
        }
        // 3. clear trường keyword
        $scope.model.keywords = "";

    }
    // adđ keywords comment
    $scope.addKeyword = function () {
        // 1. kiểm tra keyword có hay chưa
        var valueKeyword = $scope.model.keywords;
        // nếu keyword chưa có 
        if ($scope.ConditionStatement.Comment_post.keywords.findIndex(x => x == valueKeyword) == -1) {
            // 2. thêm keyword vào mảng
            $scope.ConditionStatement.Comment_post.keywords.push(valueKeyword);
        }
        // 3. clear trường keyword
        $scope.model.keywords = "";
    ...
    dataservice.getbotsocial(2,function (rs) {
        rs = rs.data;
        $scope.listbot = rs;
    });
    $scope.initData();
