var ctxfolder = "/views/admin/assetPackManager";
...
app.factory('dataservice', function ($http) {
    ...
    return {
        ...
        getTreeData: function (callback) {
            $http.post('/Admin/AssetPackManager/GetTreeData').then(callback);
        },
        ...
        insertPackAttribute: function (data, callback) {
            $http.post('/Admin/AssetPackManager/InsertPackAttribute', data).then(callback);
        },
        updatePackAttribute: function (data, callback) {
            $http.post('/Admin/AssetPackManager/UpdatePackAttribute', data).then(callback);
        },
        getItemPackAttr: function (data, callback) {
            $http.post('/Admin/AssetPackManager/GetItemPackAttr?id=' + data).then(callback);
        },
        deletePackAttribute: function (data, callback) {
            $http.post('/Admin/AssetPackManager/DeletePackAttribute?id=' + data).then(callback);
        },

        //Records Pack
        insertRecordsPack: function (data, callback) {
            $http.post('/Admin/AssetPackManager/InsertRecordsPack', data).then(callback);
        },

        //Records Pack Arrange
        getRecordsPack: function (data1, data2, callback) {
            $http.get('/Admin/AssetPackManager/GetRecordsPack?packType=' + data1 + '&level=' + data2).then(callback);
        },
        ...
        encapsulateRecordsPack: function (data, callback) {
            $http.post('/Admin/AssetPackManager/EncapsulateRecordsPack', data).then(callback);
        },
        autoUpdateHierarchy: function (callback) {
            $http.post('/Admin/AssetPackManager/AutoUpdateHierarchy').then(callback);
        },

        //Arrange
        arrangeRecordsPack: function (data, callback) {
            $http.post('/Admin/AssetPackManager/ArrangeRecordsPack', data).then(callback);
        },
        checkArrangeRecordPack: function (data, callback) {
            $http.post('/Admin/AssetPackManager/CheckArrangeRecordPack?packCode=' + data).then(callback);
        },

        //Delete Record Pack
        deleteRecordsPack: function (data, callback) {
            $http.post('/Admin/AssetPackManager/DeleteRecordsPack?id=' + data).then(callback);
        },

        //Update Records Pack
        getItemRecordsPack: function (data, callback) {
            $http.post('/Admin/AssetPackManager/GetItemRecordsPack?id=' + data).then(callback);
        },
        updateRecordsPack: function (data, callback) {
            $http.post('/Admin/AssetPackManager/UpdateRecordsPack', data).then(callback);
        },

        //File Pack Record
        getFileEdms: function (callback) {
            $http.post('/Admin/AssetPackManager/GetFileEdms').then(callback);
        },
        insertRecordsFilePack: function (data, callback) {
            $http.post('/Admin/AssetPackManager/InsertRecordsFilePack', data).then(callback);
        },
        deleteRecordFile: function (data, callback) {
            $http.post('/Admin/AssetPackManager/DeleteRecordFile?id=' + data).then(callback);
        }
    }
});
...
app.config(function ($routeProvider, $validatorProvider, $translateProvider) {
    $translateProvider.useUrlLoader('/Admin/AssetPackManager/Translation');
    ...
    $scope.col_defs = [
        {
            ...
            sortable: true,
            sortingType: "string",
            displayName: "Nhãn tài sản",
            cellTemplate: "<i class=''>{{row.branch[col.field]}}</i>"
        },
        {
            field: "Quantity",
            displayName: "Số lượng tài sản",
            sortable: true,
            sortingType: "string",
...
app.controller('recordsAttribute', function ($scope, $rootScope, $compile, $uibModal, DTOptionsBuilder, DTColumnBuilder, $confirm, $uibModalInstance, dataservice) {
    ...
    vm.dtOptions = DTOptionsBuilder.newOptions()
        .withOption('ajax', {
            url: "/Admin/AssetPackManager/JTablePackAttribute",
...
app.controller('add-pack-records', function ($scope, $rootScope, $confirm, DTOptionsBuilder, $compile, $uibModal, $uibModalInstance, DTColumnBuilder, DTInstances, dataservice, $translate, $window) {
    ...
    vm.dtOptions = DTOptionsBuilder.newOptions()
        .withOption('ajax', {
            url: "/Admin/AssetPackManager/JTableRecordsPack",
            beforeSend: function (jqXHR, settings) {
                App.blockUI({
                    target: "#contentMain",
                    boxed: true,
    ...
    vm.dtColumns.push(DTColumnBuilder.newColumn('PackAttrCode').withTitle('{{"Mã & tên thuộc tính" | translate}}').renderWith(function (data, type, full) {
...
app.controller('record-pack-files', function ($scope, $rootScope, $confirm, DTOptionsBuilder, $compile, $uibModal, DTColumnBuilder, DTInstances, dataservice, $translate) {
    ...
    $scope.submit = function () {
        ...
        if (!$rootScope.isAdded) {
            return App.toastrError(caption.COM_MSG_PACK_IS_NOT_ADDED);
        }
    ...
    vm.dtOptions = DTOptionsBuilder.newOptions()
        .withOption('ajax', {
            url: "/Admin/AssetPackManager/JTableFileRecordPack",
            beforeSend: function (jqXHR, settings) {
                App.blockUI({
                    target: "#contentMain",
                    boxed: true,
...
app.controller('records-pack-arrange', function ($scope, $rootScope, $confirm, DTOptionsBuilder, $compile, $uibModal, $uibModalInstance, DTColumnBuilder, DTInstances, dataservice, $translate, $window) {
    ...
    $scope.addBellow = function () {
        ...
        if ($scope.model.PackCode == "" || $scope.model.PackCode == null || $scope.model.PackCode == undefined) {
            return App.toastrError(caption.COM_MSG_PACK_CODE_NOT_EXIST);
        }
        ...
        if (!check) {
            ...
            App.toastrError(caption.SVC_MSG_PACK_CODE_EXIST);
        }
    }
    ...
    $scope.selectChange = function (SelectType, item) {
        if (SelectType == "PackType" && $scope.model.PackType != "") {
            ...
            dataservice.getRecordsPack($scope.model.PackType, $scope.model.PackLevel, function (rs) {
                rs = rs.data;
                $scope.recordsPacks = rs;
            })
        }
        ...
        if (SelectType == "PackCode") {
            $scope.model.PackParent = item.PackParent;
            $scope.packName = item.PackName;
            $scope.packCode = item.PackCode;
            $scope.packParentName = "";
            for (var i = 0; i < $scope.lstRecordsPackAll.length; i++) {
                if ($scope.lstRecordsPackAll[i].PackCode === $scope.model.PackParent) {
                    $scope.packParentName = $scope.lstRecordsPackAll[i].PackName;
                    break;
                }
            }
        }
    ...
    $scope.recordsPack = function () {
        ...
        if ($scope.model.RecordsPack == "" || $scope.model.RecordsPack == null || $scope.model.RecordsPack == undefined) {
            return App.toastrError(caption.COM_MSG_RECORD_PACK_NOT_EXIST);
        }
        ...
        App.toastrSuccess(caption.COM_MSG_RECORD_PACK_SUCCESS);
    }
    ...
    $scope.arrangePack = function () {
        ...
        if (isOnePack > 1) {
            return App.toastrError(caption.COM_MSG_RECORD_PACK_TOO_BIG);
        }
...
app.controller('detail', function ($scope, $rootScope, $compile, $uibModal, $uibModalInstance, DTOptionsBuilder, DTColumnBuilder, DTInstances, dataservice, $filter, para) {
    ...
    $scope.add = function () {
        if ($scope.model.ValueSet == '') {
            App.toastrError(caption.COM_MSG_VALUE_SET_NOT_EXIST);
