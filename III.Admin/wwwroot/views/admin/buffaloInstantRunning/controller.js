var ctxfolderBuffaloChannel = "/views/admin/buffaloInstantRunning";
...
app.factory('dataservice', function ($http) {
    ...
    return {
        edit: function (data, callback) {
            $http.post('/Admin/BuffaloInstantRunning/UpdateAll?id=' + data).then(callback);
        },
        delete: function (data, callback) {
            $http.post('/Admin/BuffaloInstantRunning/Delete?id=' + data).then(callback);
        },
        getListData: function (data, callback) {
            $http.post('/Admin/BuffaloInstantRunning/GetListData?id=' + data).then(callback);
        },
        insert: function (data, callback) {
            $http.post('/Admin/BuffaloInstantRunning/Insert', data).then(callback);
        },
    };
});
...
app.controller('index', function ($scope, $rootScope, $sce, $location, $compile, $uibModal, DTOptionsBuilder, DTColumnBuilder, DTInstances, dataservice, $filter, $translate, $timeout) {
    ...
    vm.dtOptions = DTOptionsBuilder.newOptions()
        .withOption('ajax', {
            url: "/Admin/BuffaloInstantRunning/JTable",
    ...
    vm.dtColumns.push(DTColumnBuilder.newColumn('Id').withTitle('{{"Id" | translate}}').withOption('sClass', 'nowrap w20').renderWith(function (data, type) {
        return data;
    }));
    vm.dtColumns.push(DTColumnBuilder.newColumn('MachineId').withTitle('Machine Id').withOption('sClass', 'nowrap w100').renderWith(function (data, type) {
        return "<a>" + data + "</a>";
    }));
   /* vm.dtColumns.push(DTColumnBuilder.newColumn('ChannelId').withTitle('{{"Channel Id" | translate}}').withOption('sClass', 'nowrap w200').renderWith(function (data, type, full) {
        return data;
    }));*/
    /*"Id", "MachineId", "ChannelId", "Ip", "LastSeen", "Status", "Flag", "CreatedTime"*/
    vm.dtColumns.push(DTColumnBuilder.newColumn('Ip').withTitle('{{"Ip" | translate}}').withOption('sClass', 'nowrap w100').renderWith(function (data, type) {
        return data;
    }));
   /* vm.dtColumns.push(DTColumnBuilder.newColumn('LastSeen').withTitle('{{"Join Time" | translate}}').withOption('sClass', 'nowrap w100').renderWith(function (data, type) {
        return data != "" ? $filter('date')(new Date(data), 'dd/MM/yyyy') : null;
    }));*/
    vm.dtColumns.push(DTColumnBuilder.newColumn('Status').withTitle('{{"Trạng Thái" | translate}}').withOption('sClass', 'nowrap w50').renderWith(function (data, type) {
        if (data == "True") {
    ...
    /*vm.dtColumns.push(DTColumnBuilder.newColumn('Flag').withTitle('{{"Flag" | translate}}').withOption('sClass', 'nowrap w100').renderWith(function (data, type) {
        return data;
    }));
    vm.dtColumns.push(DTColumnBuilder.newColumn('CreatedTime').withTitle('{{"Created Time" | translate}}').withOption('sClass', 'nowrap w100').renderWith(function (data, type) {
        return data;
    }));*/
    vm.dtColumns.push(DTColumnBuilder.newColumn("Action").withTitle("Action").withOption('sClass', 'nowrap w100').notSortable()
        .renderWith(function (data, type, full, meta) {
            // Id not id
            /*$scope.selected[full.Id] = false;*/
            return '  <a title="Schedule" ng-click="schedule(' + full.Id + ')" ><i style="--fa-primary-color: green;" class="fa-solid fa-calendar-days fs25 margin_right"></i></a> ' +
                '  <a title="Command" ng-click="command(' + full.Id + ')" ><i style="--fa-primary-color: green;" class="fa-solid fa-terminal fs25 margin_right"></i></a> ' + 
                '  <a title="Stop" ng-click="stop(' + full.Id + ')" ><i style="--fa-primary-color: green;" class="fa-solid fa-circle-stop fs25 margin_right"></i></a> ';
    ...
    $scope.command = function (id) {
        dataservice.getListData(id, function (rs) {
            ...
            if (rs.Object === undefined || rs.Object === null || rs.Object === '') {
                ...
                var modalInstance = $uibModal.open({
                    animation: true,
                    templateUrl: ctxfolderBuffaloChannel + '/command.html',
                    controller: 'command',
                    backdrop: 'static',
                    size: '60',
                    resolve: {
...
app.controller('command', function ($scope, $rootScope, $confirm, $compile, $uibModal, $uibModalInstance, DTOptionsBuilder, DTColumnBuilder, DTInstances,
    dataservice, $filter, para) {
    var vm = $scope;
    $scope.model = {
        Id: para.Id,
    ...
    $scope.ListCommand = [
        { Code: 0, Name: "Command Action ID 1" },
        { Code: 1, Name: "Command Action ID 2" },
        { Code: 2, Name: "Command Action ID 3" },
        { Code: 3, Name: "Command Action ID 4" }
        ]
    $scope.initData();
    $scope.submit = function () {
        $scope.model.ListKeyWord = JSON.stringify($scope.model.ListKeyWord);
