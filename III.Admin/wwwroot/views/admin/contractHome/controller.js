...
app.factory('dataservice', function ($http) {
    ...
    return {
        ...
        getCountContractHome: function (callback) {
            $http.get('/Admin/Contract/GetCountInContractHome').then(callback);
        }
...
app.config(function ($routeProvider, $validatorProvider, $translateProvider, $qProvider) {
    $translateProvider.useUrlLoader('/Admin/UserBusyOrFree/TranslationContractHome');
...
app.controller('index', function ($scope, $rootScope, $sce, $compile, $uibModal, DTOptionsBuilder, DTColumnBuilder, DTInstances, dataservice, $filter, $translate) {
    $scope.initBoxCard = function () {
        dataservice.getCountContractHome(function (rs) {
            rs = rs.data;
            $scope.pending = rs.Pending;
            $scope.all = rs.All;

            $scope.pendingPo = rs.PendingPO;
            $scope.allPO = rs.AllPO;

            $scope.rqImpPending = rs.RqImpPending;
            $scope.allRqImp = rs.AllRqImp;

            $scope.rqPricePending = rs.RqPricePending;
            $scope.allRqPrice = rs.AllRqPrice
        });

        dataservice.getCountBuyer(function (rs) {
            rs = rs.data;
        ...
        dataservice.amchartCountBuy(function (rs) {
            ...
            for (var i = 0; i < rs.length; i++) {
                ...
                monthbuy.push(caption.CHC_LBL_MONTH + ' ' + (rs[i].Month));
            }
            setTimeout(function () {
                var chart = c3.generate({
                    ...
                    data: {
                        ...
                        names: {
                            // name of each serie
                            'sum': caption.CHC_LBL_TOTAL_NUMBERS,
                            'success': caption.CHC_LBL_FINISH,
                            'pend': caption.CHC_LBL_PROCESSING,
                            'cancel': caption.CHC_LBL_CANCEL,
                            //'data5': 'Hủy bỏ',
                        }
                    },
                    axis: {
        ...
        dataservice.getCountSale(function (rs) {
            rs = rs.data;
        ...
        dataservice.amchartCountSale(function (rs) {
            ...
            for (var i = 0; i < rs.length; i++) {
                ...
                monthsale.push(caption.CHC_LBL_MONTH + ' ' + (rs[i].Month));
            }
    ...
    $scope.switchDiv = function () {
        ...
        divMenuDigital = $('#menuContractHome');
        divChart = $('#chartContractHome');
