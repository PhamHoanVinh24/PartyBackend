...
app.factory('dataservice', function ($http) {
    ...
    return {
        ...
        getHistoryProductInStore: function (data, callback) {
            $http.post('/Admin/MaterialExportInfoProduct/GetHistoryProductInStore?id='+data).then(callback);
        },
    }
});
...
app.controller('index', function ($scope, $rootScope, $compile, $uibModal, DTOptionsBuilder, DTColumnBuilder, DTInstances, dataservice, $translate) {
    ...
    $scope.search = function () {
        if ($scope.model.Id != '') {
            dataservice.getInfoProductInStore($scope.model.Id, function (result) {
                result = result.data;
                $scope.productInfo = result[0].ObjInStock;
                $scope.lstPosition = result[0].ListMapping;
                $scope.showInfo = true;
                $scope.getHistoryProductInStore();
                $rootScope.ObjCode = $scope.productInfo.ProductQrCode;
            });
        } else {
            App.toastrError(caption.MEXIP_MSG_CHOOSE_ID)
        }
    }
    $scope.getHistoryProductInStore = function() {
        if ($scope.model.Id != '') {
            dataservice.getHistoryProductInStore($scope.model.Id, function (result) {
                result = result.data;
                $scope.lstHistory = result;
            });
        } else {
            App.toastrError(caption.MEXIP_MSG_CHOOSE_ID)
        }
    ...
    $scope.locateProduct = function (position) {

        location.href = "/Admin/EDMSDiagramWarehouse?mapping=" + position;
    }
});
