...
app.factory('dataserviceBuy', function ($http) {
    ...
    return {
        ...
        getTotalMoney: function (data, callback) {
            $http.get('/Admin/AssetBuy/GetTotalMoney?ticketCode=' + data).then(callback);
        },

        //Workfow
        getListRepeat: function (data, callback) {
            $http.post('/Admin/AssetBuy/GetListRepeat?code=' + data).then(callback);
        },
        ...
        getItemHeaderWithCode: function (data, callback) {
            $http.post('/Admin/AssetBuy/GetItemHeaderWithCode?code=' + data).then(callback);
        },
        getActionStatus: function (data, callback) {
            $http.get('/Admin/AssetBuy/GetActionStatus?code=' + data).then(callback);
        },
        getItemHeader: function (data, callback) {
            $http.post('/Admin/AssetBuy/GetItemHeader?id=' + data).then(callback);
        },
        getListRepeat: function (data, callback) {
            $http.post('/Admin/AssetBuy/GetListRepeat?code=' + data).then(callback);
        },
        getStepWorkFlow: function (data, callback) {
            $http.post('/Admin/AssetBuy/GetStepWorkFlow?code=' + data).then(callback);
        },
        ...
        getAsset: function (data, callback) {
            $http.post('/Admin/AssetBuy/GetAsset?ticketCode=' + data).then(callback);
        },
        getLogStatus: function (data, callback) {
            $http.post('/Admin/AssetBuy/GetLogStatus?code=' + data).then(callback);
        },
...
app.controller('Ctrl_ESEIM_BUY', function ($scope, $rootScope, $compile, $uibModal, dataserviceBuy, $cookies, $translate) {
    ...
    $rootScope.ObjectTypeFile = "ASSET_BUY";
...
app.controller('index', function ($scope, $rootScope, $compile, $uibModal, DTOptionsBuilder, DTColumnBuilder, DTInstances, dataserviceBuy, $filter, $location, myService) {
    $scope.modelsearch = {
        TicketCode: '',
        Title: '',
        FromDate: '',
...
app.controller('assetRMT', function ($scope, $rootScope, $confirm, $compile, $uibModal, DTOptionsBuilder, DTColumnBuilder, DTInstances, dataserviceBuy, $filter, $translate, myService, $location) {
    var vm = $scope;
    ...
    vm.dtColumns.push(DTColumnBuilder.newColumn('Currency').withTitle('{{ "ASSET_BUY_CURRENCY" | translate }}').renderWith(function (data, type) {
        return data;
    }));
    vm.dtColumns.push(DTColumnBuilder.newColumn('Status').withTitle('{{ "ASSET_BUY_STT" | translate }}').renderWith(function (data, type, full) {
        var status = "";
    ...
    vm.dtColumns.push(DTColumnBuilder.newColumn('action').withTitle('{{"ASSET_BUY_LIST_COL_ACTION" | translate}}').renderWith(function (data, type, full) {
        return '<span style="cursor: pointer" title="{{&quot; COM_BTN_EDIT &quot; | translate}}" ng-click="edit(' + full.ID + ')"  class="fs25 pr10"><i class="fas fa-edit"style="--fa-primary-color: green;"></i></span>' +
            '<span style="cursor: pointer" title="{{&quot; COM_BTN_DELETE &quot; | translate}}" ng-click="delete(' + full.ID + ')"  class="fs25"><i class="fas fa-trash"style="--fa-primary-color: red;"></i></span>';
    }));
...
app.controller('add', function ($scope, $rootScope, $compile, $uibModal, DTColumnBuilder, DTInstances, dataserviceBuy, $filter) {
    $scope.CheckCode = '';
    $scope.model = {
        Buyer: '',
        Depart: '',
    ...
    $scope.modelWf = {
        WorkflowCode: "",
        ObjectType: "ASSET_BUY",
        ObjectInst: "",
    };

    $rootScope.TotalMoneyBuy = 0;
    $rootScope.Currency = '';
    $scope.initLoad = function () {
        ...
        dataserviceBuy.getWorkFlow(function (result) {
            result = result.data;
            $rootScope.lstWorkflow = result;
        });
        dataserviceBuy.getStatusAct(function (rs) {
            rs = rs.data;
            $scope.lstStatus = rs;
        })

        dataserviceBuy.suggestWF($scope.modelWf.ObjectType, function (rs) {
            rs = rs.data;
    ...
    $scope.changeWorkFlowInts = function (a) {
        dataserviceBuy.getStepWorkFlow(a, function (rs) {
            data = rs.data;
    ...
    $scope.changeStatus = function () {
        if ($scope.model.Status == "REPEAT_REQUIRE_REWORK" || $scope.model.Status == "FINAL_REQUIRE_REWORK") {
            dataserviceBuy.getListRepeat($scope.model.TicketCode, function (rs) {
                rs = rs.data;
                var arr = [];
            ...
            if (!$rootScope.IsAdd) {
                dataserviceBuy.insert($scope.model, function (result) {
                    ...
                    if (result.Error) {
                        ...
                        $rootScope.ObjCode = $scope.model.TicketCode;
                        $rootScope.TotalMoneyBuy = $scope.model.TotalMoney;
                        ...
                        dataserviceBuy.createWfInstance($scope.modelWf, function (rs) {
                            rs = rs.data;
                            if (rs.Error) {
                                App.toastrError(rs.Title);
                            } else {
                                ...
                                dataserviceBuy.getLogStatus($scope.model.TicketCode, function (rs) {
                                    rs = rs.data;
    ...
    $scope.editInstAct = function (id, objCode) {
        dataserviceBuy.checkPermissionEditActivityById(id, function (rs) {
            rs = rs.data;
            if (!rs) {
                return App.toastrError(caption.COM_MSG_PERMISSION_EDIT_ACT);
            }
            dataserviceBuy.getItemActInst(id, function (rs) {
...
app.controller('addtable', function ($scope, $rootScope, $confirm, $compile, $uibModal, DTOptionsBuilder, DTColumnBuilder, DTInstances, dataserviceBuy, $filter, $translate) {
    ...
    $scope.model = {
        ...
        Supplier: '',
        CostValue: '',
        CurrencyAsset: $rootScope.Currency
    };
    $scope.selected = [];
    $scope.selectAll = false;
    $scope.toggleAll = toggleAll;
    ...
    vm.dtOptions = DTOptionsBuilder.newOptions()
        .withOption('ajax', {
            ...
            complete: function () {
                App.unblockUI("#contentMain");
                heightTableManual(302, "#tblDataAssetBuy");
            }
        })
    ...
    vm.dtColumns.push(DTColumnBuilder.newColumn('Currency').withTitle('{{"ASSET_BUY_TYPE_CURRENCY" | translate}}').withOption('sWidth', '30px').renderWith(function (data, type) {
        return data;
    }));
    ...
    $scope.initData = function () {
        dataserviceBuy.getTotalMoney($rootScope.TicketCode, function (rs) {
            rs = rs.data;
            $scope.total = rs;
        })
    }
    ...
    $rootScope.fixMoney = function () {
        $scope.model.CurrencyAsset = $rootScope.Currency;
    }
    ...
    $scope.changeTotalMoney = function () {

        if ($scope.model.CostValue > ($rootScope.TotalMoneyBuy - $scope.total)) {
            App.toastrError(caption.ASSET_BUY_COST_VALUE_TOO_BIG)
            $scope.model.CostValue = 0;
        }
    }
...
app.controller('editBuy', function ($scope, $rootScope, $compile, $uibModal, dataserviceBuy, $filter, $location, myService) {
    ...
    if (para != undefined) {
        ...
        $scope.initData = function () {
            dataserviceBuy.getItemBuy(para, function (rs) {
                ...
                dataserviceBuy.getAsset($rootScope.TicketCode, function (rs) {
                    rs = rs.data;
                    $scope.lstProd = rs;
                })
                setTimeout(function () {
        ...
        if ($scope.editform.validate() && !validationSelect($scope.model).Status) {
            dataserviceBuy.updateBuy($scope.model, function (rs) {
                ...
                if (rs.Error) {
                    ...
                    dataservice.updateStatusWF("ASSET_BUY", $scope.model.TicketCode, $scope.model.Status, $scope.model.ActRepeat, function (rs) {
                        if ($scope.model.Status != "FINAL_DONE") {
                            dataserviceBuy.getActionStatus($scope.model.TicketCode, function (rs) {
                                rs = rs.data;
                            ...
                            dataserviceBuy.getItemHeaderWithCode($scope.model.TicketCode, function (rs) {
                                data = rs.data;
                                $scope.lstStep = data.list;
    ...
    $scope.print = function () {
        ...
        var infoTime = $scope.model.BuyTime.split("/");

        var personRecei = "";
        for (var i = 0; i < $scope.ListBuyer.length; i++) {
            if ($scope.model.Buyer === $scope.ListBuyer[i].Code) {
                personRecei = $scope.ListBuyer[i].Name;
                break;
            }
        }
        ...
        var content = '<p><strong>Đơn vị:</strong>................................. </p>' +
            ...
            '<p></p>' +
            '<p style="text-align: center;"><strong>PHIẾU MUA SẮM TÀI SẢN</strong></p>' +
            ...
            '<p>Người mua: <strong>' + personRecei + '</strong></p>' +
            '<p>Địa chỉ: ' + $scope.model.Location + '</p>' +
...
app.controller('fileAsset', function ($scope, $rootScope, $compile, $uibModal, DTOptionsBuilder, DTColumnBuilder, DTInstances, dataserviceBuy, $filter, dataserviceSupplier) {
    var vm = $scope;
    $scope.selected = [];
    $scope.selectAll = false;
    $scope.toggleAll = toggleAll;
    ...
    $scope.extend = function (id) {
        ...
        dataserviceSupplier.getDefaultRepo($rootScope.AssetCode, 'ASSET_BUY', function (rs) {
            rs = rs.data;
            var data = rs !== null ? rs : { CatCode: '', ObjectCode: $rootScope.AssetCode, ObjectType: 'ASSET_BUY' };
            var modalInstance = $uibModal.open({
...
app.controller('tabFileHistory', function ($scope, $rootScope, $compile, $uibModal, $uibModalInstance, DTOptionsBuilder, DTColumnBuilder, DTInstances, dataserviceBuy, $filter, para) {
    ...
    $scope.getObjectFile = function (id, mode) {
        dataserviceBuy.getItemFile(id, true, mode);
    };
