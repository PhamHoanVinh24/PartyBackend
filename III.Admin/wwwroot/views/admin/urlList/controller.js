var ctxfolderURLList = "/views/admin/urlList";
var ctxfolderLmsDashBoard = "/views/admin/lmsDashBoard";
var ctxfolderURLListMessage = "/views/message-box";
...
app.factory('dataservice', function ($http) {
    ...
    return {
        ...
        delete: function (data, callback) {
            $http.post('/Admin/UrlList/Delete?id=' + data).then(callback);
        },
        getListSubject: function (callback) {
            $http.post('/Admin/LmsCourse/GetListSubject').then(callback);
        },
        insert: function (data, callback) {
            $http.post('/Admin/UrlList/Insert', data).then(callback);
        },
        updateAll: function (data, callback) {
            $http.post('/Admin/UrlList/UpdateAll/', data).then(callback);
        },
    };
});
...
app.config(function ($routeProvider, $validatorProvider, $translateProvider, $qProvider) {
    $translateProvider.useUrlLoader('/Admin/URLList/Translation');
    caption = $translateProvider.translations();
    $qProvider.errorOnUnhandledRejections(false);
    $routeProvider
        .when('/', {
            templateUrl: /*ctxfolderLmsDashBoard + '/index.html'*/ctxfolderURLList + '/index-course.html',
            controller: 'index'
        })
...
app.controller('index', function ($scope, $rootScope, $sce, $compile, $uibModal, DTOptionsBuilder, DTColumnBuilder, DTInstances, dataservice, $filter, $translate) {
    ...
    //    return data;
    //}));
    vm.dtColumns.push(DTColumnBuilder.newColumn('Url').withTitle('{{"Url" | translate}}').withOption('sClass', 'nowrap w300').renderWith(function (data, type) {
        return data;/* URL_LIST_TXT_DOMAIN_NAME*/
    }));
    vm.dtColumns.push(DTColumnBuilder.newColumn('DownloadFile').withTitle('{{"DownloadFile" | translate}}').withOption('sClass', 'nowrap w300').renderWith(function (data, type) {
        if (data == "True") {
    ...
    //    return data + ' ' + unit;
    //}));
    vm.dtColumns.push(DTColumnBuilder.newColumn('KeyWord').withTitle('{{"KeyWord" | translate}}').withOption('sClass', 'nowrap w30').renderWith(function (data, type) {
        return data;
    }));/* URL_LIST_TXT_STATUS*/
    ...
    //    return status;
    //}));
    vm.dtColumns.push(DTColumnBuilder.newColumn('DataStorage').withTitle('{{"DataStorage" | translate}}').withOption('sClass', 'nowrap w150').renderWith(function (data, type) {
        return data; /*URL_LIST_TXT_TIME_TOTAL*/
    }));
    vm.dtColumns.push(DTColumnBuilder.newColumn('DomainId').withTitle('{{"DomainId" | translate}}').withOption('sClass', 'nowrap w150').renderWith(function (data, type) {
        return data; /*URL_LIST_TXT_COUNT_TIME*/
    }));
    ...
    $scope.add = function () {
        var modalInstance = $uibModal.open({
            animation: true,
            templateUrl: ctxfolderURLList + '/addd.html',
            controller: 'addd',
            backdrop: 'static',
            size: '50'
        });
    ...
    $scope.edit = function (id) {
        ...
        var modalInstance = $uibModal.open({
            animation: true,
            templateUrl: ctxfolderURLList + '/edit.html',
            controller: 'edit',
            backdrop: 'static',
            size: '60',
            resolve: {
    ...
    $scope.popupVideo = function (id) {
        ...
        dataservice.getSubject(id, function (rs) {
            ...
            var modalInstance = $uibModal.open({
                animation: true,
                templateUrl: ctxfolderURLList + '/popup-video.html',
    ...
    $scope.delete = function (id) {
        var modalInstance = $uibModal.open({
            templateUrl: ctxfolderURLListMessage + '/messageConfirmDeleted.html',
    ...
    $scope.viewLectureDetail = function (item) {
        var modalInstance = $uibModal.open({
            animation: true,
            templateUrl: ctxfolderURLList + '/viewLecture.html',
    ...
    $scope.viewQuestionDetail = function (item) {
        var modalInstance = $uibModal.open({
            animation: true,
            templateUrl: ctxfolderURLList + '/viewQuestion.html',
    ...
    $scope.chooseLecture = function (index) {
        if ($scope.isVideoPlaying) {
            ...
            dataservice.logSession(session, function (rs) {
                ...
                App.toastrInfo("Bạn đã kết thúc bài giảng");
            });
        }
...
app.controller('edit', function ($scope, $rootScope, $confirm, $compile, $uibModal, $uibModalInstance, DTOptionsBuilder, DTColumnBuilder, DTInstances, dataservice, $filter, para) {
    ...
    $scope.model = {
        Id: para.Id,
        Url: para.Url,
        DownloadFile: para.DownloadFile,
        KeyWord: para.KeyWord,
        DomainId: para.DomainId,
        DataStorage: para.DataStorage
    }
...
app.controller('addd', function ($scope, $rootScope, $confirm, $compile, $uibModal, $uibModalInstance, DTOptionsBuilder, DTColumnBuilder, DTInstances, dataservice, $filter) {
    var vm = $scope;
    $scope.model = {
        Id: '',
        Url: '',
...
app.controller('viewLecture', function ($scope, $rootScope, $compile, $uibModal, DTOptionsBuilder, DTColumnBuilder, DTInstances, $uibModalInstance, $filter, $translate, dataservice, para) {
    ...
    $scope.viewFileDetail = function (item) {
        var modalInstance = $uibModal.open({
            animation: true,
            templateUrl: ctxfolderURLList + '/viewFile.html',
            controller: 'viewFile',
            backdrop: 'static',
            size: '40',
            resolve: {
