    ...
    $scope.Runcommentpost = function () {
        if ($scope.model.Id != "") {
            ...
            dataservice.runcrawler($scope.model.Id, function (rs) {
                ...
                rs = rs.data
                login = {
                    BotSocialCode: rs.BotSocialCode,
                    BotSocialName: rs.BotSocialName,
                    UserName: rs.UserName,
                    PassWord: rs.PassWord,
                    Cookie: decodecookie(rs.Tocken)
                }
    ...
    $scope.Rununlikepost = function () {
        if ($scope.model.Id != "") {
            ...
            dataservice.runcrawler($scope.model.Id, function (rs) {
                ...
                unlike = {
                    "Unlike post": decodeHTML(data.Unlike.Post)
                }
                ws.onopen = function () {
                    //ws.send("Start");
                    ws.send(login)
...
{ 'id': 'Hang Bui', 'label': 'Hang Bui' }
    ...
    $scope.ConditionStatement = {
        ...
        Post:
        {
            Main_post:
            {
                ...
                friends: [],
                content: {
                    Image: "https://dbk.vn/uploads/news/images/Artificial-Intelligence-(ai)-la-gi-2.jpg/",
                    video: "",
                    content:""
                }
            },
    ...
    $scope.showcondition = function () {
        dataservice.getconditon($scope.model.Id, function (rs) { // bot code is set to 1
            ...
            $scope.selectedgroup_add = a.Addfriend.group
    ...
    // adđ keywwords comment post
    $scope.addKeyword = function () {
        // 1. kiểm tra keyword có hay chưa
        var valueKeyword = $scope.model.keywords;
        // nếu keyword chưa có 
        if ($scope.ConditionStatement.Post.Comment_post.keywords.findIndex(x => x == valueKeyword) == -1) {
            // 2. thêm keyword vào mảng
            $scope.ConditionStatement.Post.Comment_post.keywords.push(valueKeyword);
        }
        // 3. clear trường keyword
        $scope.model.keywords = "";
    ...
    $scope.submitbot = function () {
        ...
        if ($scope.model.Id != "") {
            ...
            $scope.ConditionStatement.Unlike.Post.Group = $scope.selectedgroup_unlike
    ...
    $scope.initData = function () {
        ...
        xmlHttp.open("GET", "https://graph.facebook.com/v13.0/158547376613081/groups?access_token=EAAPnFtt7bUMBACdu3CJrHQzMGDXTsg2RZBA4TC5MHrDQUuPw2ZCB96ZAA1sfmBK9vXBb1bUGuVw6mZAuJz1yORbbx5AF70tNPwhxGDF8zWrl9OvjZBY3aYnIm3iNDJBD5TsJ5MdjYAvfTs7JzcnFczo7qJrv7ylrZAn91VkvY1ywZDZD&pretty=0&limit=500&after=QVFIUkxDWTlYbzRRSmJkRXhyQW9iVy1DNUNXSGZALNHZAZAYlRtMXFqTl9TMVd0XzFleUZAFZADFEN1k1eWxFc0FhRHFDOTlCSHZAuRVRrVFFaelo1ZAUd4UVRXS1ZAn", false); // false for synchronous request
        xmlHttp.send(null);
