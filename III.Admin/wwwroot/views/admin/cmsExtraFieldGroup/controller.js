...
app.factory('dataservice', function ($http) {
    ...
    return {
        ...
        getItem: function (data, callback) {
            $http.post('/Admin/CMSExtraFieldGroup/GetItem/', data).then(callback);
        },
        insert: function (data, callback) {
            $http.post('/Admin/CMSExtraFieldGroup/Insert/', data).then(callback);
        },
        update: function (data, callback) {
            $http.post('/Admin/CMSExtraFieldGroup/Update/', data).then(callback);
        },
        delete: function (data, callback) {
            $http.post('/Admin/CMSExtraFieldGroup/Delete/' + data).then(callback);
        },
            ...
            messages: {
               Name: {
                  required: "Tên không được để trống"
                   //required: caption.CMS_EFG_VALIDATE_NAME,
                }
            }
        }
...
app.config(function ($routeProvider, $validatorProvider, $translateProvider) {
    $translateProvider.useUrlLoader('/Admin/CMSExtraFieldGroup/Translation');
...
app.controller('index', function ($scope, $rootScope, $compile, $uibModal, DTOptionsBuilder, DTColumnBuilder, DTInstances, dataservice, $translate) {
    ...
    vm.dtColumns.push(DTColumnBuilder.newColumn('name').withOption('sClass', 'nowrap dataTable-90per').withTitle('{{"CMS_EFG_LIST_COL_NAME" | translate}}').renderWith(function (data, type) {
        return data;
    }));
    vm.dtColumns.push(DTColumnBuilder.newColumn('action').notSortable().withTitle('{{ "CMS_EFG_LIST_COL_ACTION" | translate }}').renderWith(function (data, type, full, meta) {
...
app.controller('detail', function ($scope, $rootScope, $confirm, $compile, $uibModal, $uibModalInstance, DTOptionsBuilder, DTColumnBuilder, DTInstances, dataservice, $filter, para) {
    ...
    $scope.delete = function (id) {
        var modalInstance = $uibModal.open({
            ...
            windowClass: "message-center",
            controller: function ($scope, $uibModalInstance) {
                $scope.message = caption.CMS_EFG_MSG_DELETE;
                $scope.ok = function () {
