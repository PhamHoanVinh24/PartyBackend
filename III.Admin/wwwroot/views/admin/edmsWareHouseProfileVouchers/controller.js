...
app.factory('dataservice', function ($http) {
    ...
    return {
        ...
        getTreeDataParent: function (callback) {
            $http.post('/Admin/EDMSWareHouseProfileVouchers/GetTreeDataParent/').then(callback);
        },
    };
});
...
app.controller('Ctrl_ESEIM_Warehouse', function ($scope, $rootScope, $compile, $cookies, $translate) {
    $rootScope.go = function (path) {
        $location.path(path); return false;
    };
    $rootScope.PERMISSION_EDMSWareHouseProfileVouchers = /*PERMISSION_EDMSWareHouseProfileVouchers*/true;
        ...
        $rootScope.validationOptions = {
            ...
            messages: {
                Code: {
                    ...
                    regx: 'Mã không chứa khoảng trắng, dấu và ký tự đặc biệt'
                },
                Name: {
                    required: caption.EDMSWHPV_VALIDATE_NAME,
                    regx: 'Không nhập khoảng trắng đầu dòng'
                },
                Address: {
    ...
    $rootScope.ObjectTypeFile = "WAREHOUSE";
    $rootScope.moduleName = "WAREHOUSE";
});
...
app.config(function ($routeProvider, $validatorProvider, $translateProvider, $httpProvider) {
    ...
    $routeProvider
        .when('/', {
            templateUrl: ctxfolderWarehouse + '/index.html',
            controller: 'index_Warehouse'
        })
...
app.controller('index_Warehouse', function ($scope, $rootScope, $compile, $uibModal, DTOptionsBuilder, DTColumnBuilder, DTInstances, dataservice, $filter, $window) {
    var vm = $scope;
    $scope.model = {
        WHS_Code: '',
        WHS_Name: '',
    ...
    vm.dtOptions = DTOptionsBuilder.newOptions()
        ...
        .withOption('createdRow', function (row, data, dataIndex) {
            ...
            if (/*$rootScope.PERMISSION_EDMSWareHouseProfileVouchers.Update*/true) {
    ...
    vm.dtColumns.push(DTColumnBuilder.newColumn('_STT').withOption('sClass', 'wpercent1').withTitle('{{ "STT" | translate }}').renderWith(function (data, type) {
        return data;
    }));
    vm.dtColumns.push(DTColumnBuilder.newColumn('WHS_Code').withTitle('{{ "EDMSWHPV_LIST_COL_CODE" | translate }}').renderWith(function (data, type, full) {
        return `<b style="padding-left: ${15 * full.Level}px">${data}</b>`;
    }));
    ...
    vm.dtColumns.push(DTColumnBuilder.newColumn('action').notSortable().withOption('sClass', 'nowrap').withTitle('{{ "EDMSWHPV_LIST_COL_ACTION" | translate }}').withOption('sClass', '').renderWith(function (data, type, full, meta) {
        var listButton = '';
        if (/*$rootScope.PERMISSION_EDMSWareHouseProfileVouchers.Update*/true) {
            listButton += '<a title="Sửa" ng-click="edit(\'' + full.WHS_Code + '\')"  /*style = "width: 25px; height: 25px; padding: 0px;-webkit-box-shadow: 0 2px 5px 0 rgba(85,168,253,0.45);"*/ class="fs25"><i class="fas fa-edit" style="color: #336c9c;margin-right: 15px"></i></a>';
        }
        if (/*$rootScope.PERMISSION_EDMSWareHouseProfileVouchers.Delete*/true) {
            listButton += '<a title="Xoá" ng-click="delete(' + full.Id + ')" /*style="width: 25px; height: 25px; padding: 0px;-webkit-box-shadow: 0 2px 5px 0 rgba(230,60,95,0.45)"*/ class="fs25"><i class="fas fa-trash" style="color: #336c9c;--fa-primary-color: red;margin-right: 15px"></i></a>';
        }
        listButton += '<a title="Sơ đồ kho" ng-click="map(\'' + full.WHS_Code + '\')" /*style="width: 25px; height: 25px; padding: 0px;-webkit-box-shadow: 0 2px 5px 0 rgba(230,60,95,0.45)"*/ class="fs25"><i class="fas fa-draw-polygon" style="color: #336c9c;"></i></a>';
        return listButton;
    ...
    $scope.add = function () {
        ...
        var modalInstance = $uibModal.open({
            animation: true,
            templateUrl: ctxfolderWarehouse + '/add.html',
            controller: 'add',
            backdrop: 'static',
            size: '60'
        });
    ...
    $scope.edit = function (code) {
        ...
        dataservice.getDetailWareHouse(code, function (rs) {
            ...
            var modalInstance = $uibModal.open({
                animation: true,
                templateUrl: ctxfolderWarehouse + '/edit.html',
                controller: 'edit',
                backdrop: 'static',
                size: '60',
                resolve: {
    ...
    $scope.map = function(code) {
        location.href = `/Admin/EdmsDiagramWarehouse?WhsCode=${code}`;
    }
...
app.controller('add', function ($scope, $rootScope, $compile, $uibModal, $uibModalInstance, dataservice, $filter) {
            ...
            if ($rootScope.Object.WHS_Code == '') {
                dataservice.insert(formData, function (rs) {
                    ...
                    if (rs.Error) {
                        ...
                        $rootScope.ObjCode = $scope.model.WHS_Code;
                        //$uibModalInstance.close();
                    }
                });
            } else {
    ...
    $scope.openMap = function () {
        var modalInstance = $uibModal.open({
            animation: true,
            templateUrl: ctxfolderWarehouse + '/google-map.html',
...
app.controller('edit', function ($scope, $rootScope, $compile, $uibModal, $uibModalInstance, dataservice, para) {
    ...
    $scope.init = function () {
        ...
        $scope.model.ManagerId = para.ManagerId ?? '';
...
app.controller('file', function ($scope, $rootScope, $compile, $uibModal, DTOptionsBuilder, DTColumnBuilder, DTInstances, dataservice, $filter, dataserviceSupplier) {
    ...
    vm.dtColumns.push(DTColumnBuilder.newColumn('action').withTitle('{{"COM_LIST_COL_ACTION" | translate}}').withOption('sClass', 'nowrap').renderWith(function (data, type, full) {
        if (full.TypeFile == "SHARE") {
            ...
            return '<a ng-click="dowload(' + full.Id + ')" style="width: 25px; height: 25px; padding: 0px" title="Tải xuống - ' + full.FileName + '" class="btn btn-icon-only btn-circle btn-outline green"><i class="fa fa-download pt5"></i></a>' +
    ...
    $scope.search = function () {
        var modalInstance = $uibModal.open({
            templateUrl: ctxfolderWarehouse + '/fileSearch.html',
    ...
    $scope.edit = function (fileName, id) {
        dataservice.getFile(id, function (rs) {
            ...
            if (rs.Error) {
                ...
                var modalInstance = $uibModal.open({
                    animation: true,
                    templateUrl: ctxfolderWarehouse + '/file_edit.html',
    ...
    $scope.extend = function (id) {
        ...
        //    var data = rs != '' ? rs : {};
        //    var modalInstance = $uibModal.open({
        //        templateUrl: ctxfolderWarehouse + '/file_add.html',
        //        controller: 'file_add',
        //        windowClass: 'modal-file',
        //        backdrop: 'static',
        //        size: '60',
        ...
        //    }, function () { });
        //})
        dataserviceSupplier.getDefaultRepo($rootScope.Object.WHS_Code, 'MATERIAL_STORE', function (rs) {
            rs = rs.data;
            var data = rs !== null ? rs : { CatCode: '', ObjectCode: $rootScope.Object.WHS_Code, ObjectType: 'MATERIAL_STORE' };
            var modalInstance = $uibModal.open({
...
app.controller('more', function ($scope, $rootScope, $compile, $uibModal, DTOptionsBuilder, DTColumnBuilder, DTInstances, dataservice, $filter, $translate, $window) {
    ...
    $scope.add = function () {
        var modalInstance = $uibModal.open({
            animation: true,
            templateUrl: ctxfolderWarehouse + '/more_add.html',
            controller: 'more_add',
            backdrop: 'static',
            size: '20'
        });
    ...
    $scope.edit = function (id) {
        var modalInstance = $uibModal.open({
            animation: true,
            templateUrl: ctxfolderWarehouse + '/more_edit.html',
