var ctxfolder = "/views/admin/recruitmentAttributeMain";
...
app.factory('dataservice', function ($http) {
    ...
    return {
        insert: function (data, callback) {
            $http.post('/Admin/RecruitmentAttributeMain/Insert', data, callback).then(callback);
        },
        update: function (data, callback) {
            $http.post('/Admin/RecruitmentAttributeMain/Update', data).then(callback);
        },
        delete: function (data, callback) {
            $http.post('/Admin/RecruitmentAttributeMain/Delete/' + data).then(callback);
        },
        getItem: function (data, callback) {
            $http.get('/Admin/RecruitmentAttributeMain/GetItem/' + data).then(callback);
        },

        getListParent: function (data, callback) {
            $http.post('/Admin/RecruitmentAttributeMain/GetListParent?code=' + data).then(callback);
        },
        insertChildren: function (data, callback) {
            $http.post('/Admin/RecruitmentAttributeMain/InsertChildren', data, callback).then(callback);
        },
        updateChildren: function (data, callback) {
            $http.post('/Admin/RecruitmentAttributeMain/UpdateChildren', data).then(callback);
        },
        deleteChildren: function (data, callback) {
            $http.post('/Admin/RecruitmentAttributeMain/DeleteChildren/' + data).then(callback);
        },
        getItemChildren: function (data, callback) {
            $http.get('/Admin/RecruitmentAttributeMain/GetItemChildren/' + data).then(callback);
        },

        getAttrUnit: function (callback) {
            $http.post('/Admin/RecruitmentAttributeMain/GetAttrUnit/').then(callback);
        },
        getAttrGroup: function (callback) {
            $http.post('/Admin/RecruitmentAttributeMain/GetAttrGroup/').then(callback);
        },
        getAttrDataType: function (callback) {
            $http.post('/Admin/RecruitmentAttributeMain/GetAttrDataType/').then(callback);
        },
...
app.controller('Ctrl_ESEIM', function ($scope, $rootScope, $compile, $uibModal, DTOptionsBuilder, $confirm, DTColumnBuilder, DTInstances, $cookies, $translate, dataservice) {
    ...
    $rootScope.$on('$translateChangeSuccess', function () {
        ...
        $rootScope.validationOptions = {
            ...
            messages: {
                Code: {
                    required: caption.COM_ERR_REQUIRED.replace("{0}", caption.RAM_CURD_LBL_RAM_CODE),
                    regx: caption.COM_VALIDATE_ITEM_CODE.replace("{0}", caption.RAM_CURD_LBL_RAM_CODE),
                    maxlength: caption.COM_ERR_EXCEED_CHARACTERS.replace("{0}", caption.RAM_CURD_LBL_RAM_CODE).replace("{1}", "50")
                },
                Name: {
                    required: caption.COM_ERR_REQUIRED.replace("{0}", caption.RAM_CURD_LBL_RAM_NAME),
                    maxlength: caption.COM_ERR_EXCEED_CHARACTERS.replace("{0}", caption.RAM_CURD_LBL_RAM_NAME).replace("{1}", "50"),
                    regx: "Tên tiêu chí không bắt đầu bằng khoảng trắng"
                },
                Note: {
...
app.config(function ($routeProvider, $validatorProvider, $translateProvider, $httpProvider) {
    $translateProvider.useUrlLoader('/Admin/RecruitmentAttributeMain/Translation');
...
app.controller('index', function ($scope, $rootScope, $confirm, $compile, $uibModal, DTOptionsBuilder, DTColumnBuilder, DTInstances, dataservice, $translate, $window) {
    ...
    vm.dtOptions = DTOptionsBuilder.newOptions()
        .withOption('ajax', {
            url: "/Admin/RecruitmentAttributeMain/Jtable",
    ...
    vm.dtColumns.push(DTColumnBuilder.newColumn('Code').withTitle('{{"RAM_LIST_COL_RAM_CODE" | translate}}').renderWith(function (data, type) {
        return data;
    }));
    vm.dtColumns.push(DTColumnBuilder.newColumn('Name').withTitle('{{"RAM_LIST_COL_RAM_NAME" | translate}}').renderWith(function (data, type) {
        return data;
    }));
    vm.dtColumns.push(DTColumnBuilder.newColumn('Childrens').withTitle('{{"RAM_LIST_COL_RAM_CHILDREN" | translate}}').renderWith(function (data, type) {
        return data;
    }));
    vm.dtColumns.push(DTColumnBuilder.newColumn('Note').withTitle('{{"RAM_LIST_COL_RAM_DESCRIPTION" | translate}}').renderWith(function (data, type) {
        return data;
    }));
    vm.dtColumns.push(DTColumnBuilder.newColumn('action').notSortable().withOption('sClass', 'nowrap').withTitle('{{"RAM_LIST_COL_RAM_ACTION" | translate}}').renderWith(function (data, type, full) {
        var listButton = '';
        if ($rootScope.PERMISSION_MATERIAL_PRODUCT_GROUP.Update) {
            listButton += '<a ng-click="edit(' + full.Id + ')" style1= "width: 25px; height: 25px; padding: 0px" class1="btn btn-icon-only btn-circle btn-outline blue"><i class="fas fa-edit fs25 pr20"></i></a>';
        }
...
app.controller('edit', function ($scope, $rootScope, $compile, $uibModal, $confirm, $uibModalInstance, dataservice, para) {
    ...
    $scope.initData = function () {
        dataservice.getListParent(para.Code, function (rs) {
            rs = rs.data;
            $scope.listParent = rs;
        });
...
app.controller('tabAttribute', function ($scope, $rootScope, $compile, $uibModal, DTOptionsBuilder, DTColumnBuilder, DTInstances, dataservice, $filter) {
    ...
    vm.dtOptions = DTOptionsBuilder.newOptions()
        .withOption('ajax', {
            url: "/Admin/RecruitmentAttributeMain/JTableChildren",
            beforeSend: function (jqXHR, settings) {
                App.blockUI({
                    target: "#contentMain",
                    boxed: true,
            ...
            complete: function () {
                App.unblockUI("#contentMain");
                heightTableManual(150, "#tblDataAttribute");
            }
        })
        .withPaginationType('full_numbers').withDOM("<'table-scrollable't>ip")
    ...
    $scope.add = function () {
        if ($rootScope.ParentCode == '' || $rootScope.ParentCode == null || $rootScope.ParentCode == undefined) {
            App.toastrError('Vui lòng nhập tiêu chí tuyển dụng ở trên trước');
            return;
        }
