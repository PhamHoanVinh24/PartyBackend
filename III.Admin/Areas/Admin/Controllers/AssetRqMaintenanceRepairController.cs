...
namespace III.Admin.Controllers
{
    ...
    public class AssetRqMaintenanceRepairController : BaseController
    {
        ...
        public AssetRqMaintenanceRepairController(EIMDBContext context, IUploadService upload, IHostingEnvironment hostingEnvironment, IStringLocalizer<ContractController> contractController, IStringLocalizer<AssetInventoryController> inventoryController,
            ...
            IStringLocalizer<AssetRqMaintenanceRepairController> stringLocalizer, IStringLocalizer<SharedResources> sharedResources,
        ...
        [Breadcrumb("ViewData.CrumbAssetRqRepair", AreaName = "Admin", FromAction = "Index", FromController = typeof(MaintenAssetHomeController))]
        public IActionResult Index()
        {
            ...
            ViewData["CrumbAssetRqRepair"] = _sharedResources["COM_CRUMB_ASSET_RQ_MAINTENANCE"];
            return View();
        }
        ...
        public object JTable([FromBody]JTableModelAct jTablePara)
        {
            ...
            var query = (from a in _context.AssetRqMaintenanceRepairHeaders.Where(x => !x.IsDeleted)
                         ...
                         join d in _context.AdDepartments on a.CreateDepart equals d.DepartmentCode into d1
        ...
        [HttpPost]
        public object GetCreateBy(string DepartmentCode)
        {
        ...
        public JsonResult InsertLog([FromBody]ActivityLogDatamodel obj)
        {
            ...
            try
            {
                if (obj.ObjActCode != null && obj.TicketCode != null)
                {
                    ...
                    _context.SaveChanges();
                    msg.Title = String.Format(_sharedResources["COM_MSG_ADD_SUCCESS"], _stringLocalizer["ASSET_RMR_ACT"]);
                }
                else
                {
                    msg.Error = true;
        ...
        public object GetAttrName(string actcode)
        {
            ...
            var data = _context.AttrSetups.Where(x => x.ActCode == actcode).Select(x => new { Code = x.AttrCode, Name = x.AttrName }).ToList();
            return data;
        }
        [HttpPost]
        ...
        {
            var project = _context.AssetRqMaintenanceRepairHeaders.FirstOrDefault(x => x.TicketCode.Equals(code) && !x.IsDeleted);
            return Json(project);
        }
        ...
        {
            List<ComboxModel> list = new List<ComboxModel>();
            var data = _context.AssetRqMaintenanceRepairHeaders.FirstOrDefault(x => x.TicketCode.Equals(code) && !x.IsDeleted);
            var check = _context.WorkflowInstances.FirstOrDefault(x => !x.IsDeleted.Value && x.ObjectInst.Equals(data.TicketCode)
                && x.ObjectType.Equals(EnumHelper<ObjectType>.GetDisplayValue(ObjectType.AssetRqRepair)));
        ...
        {
            var data = _context.AssetRqMaintenanceRepairHeaders.Where(x => !x.IsDeleted && x.TicketCode.Equals(code)).Select(x => new
            {
                x.Status
            });
            return data;
        ...
        public object GetItemTemp(int id)
        {
            ...
            try
            {
                ...
                var data = _context.AssetRqMaintenanceRepairHeaders.FirstOrDefault(x => x.ID == id && !x.IsDeleted);
                if (data == null)
                {
                    msg.Error = true;
                    msg.Title = _stringLocalizer["ASSET_RMR_MSG_TICKET_NO_EXIST"];
                    return Json(msg);
                }
        ...
        public object JTableFile([FromBody]JTableModelFile jTablePara)
        {
            ...
            var fromDate = !string.IsNullOrEmpty(jTablePara.FromDate) ? DateTime.ParseExact(jTablePara.FromDate, "dd/MM/yyyy", CultureInfo.InvariantCulture) : (DateTime?)null;
            var toDate = !string.IsNullOrEmpty(jTablePara.ToDate) ? DateTime.ParseExact(jTablePara.ToDate, "dd/MM/yyyy", CultureInfo.InvariantCulture) : (DateTime?)null;
            var query = ((from a in _context.EDMSRepoCatFiles.Where(x => x.ObjectCode == jTablePara.AssetCode && x.ObjectType == EnumHelper<ObjectType>.GetDisplayValue(ObjectType.AssetRqRepair))
                ...
                if (Array.IndexOf(LuceneExtension.fileMimetypes, mimeType) >= 0 && (Array.IndexOf(LuceneExtension.fileExt, extension.ToUpper()) >= 0))
                {
                    ...
                    if (!obj.IsMore)
                    {
                        ...
                        var repoDefault = _context.EDMSRepoDefaultObjects.FirstOrDefault(x => !x.IsDeleted
                               && x.ObjectCode.Equals(obj.AssetCode) && x.ObjectType.Equals(EnumHelper<ObjectType>.GetDisplayValue(ObjectType.AssetRqRepair)));
                        if (repoDefault != null)
                        {
