...
{
    [Area("Admin")]
    public class RecordDeliveryController : BaseController
    {
        ...
        public RecordDeliveryController(EIMDBContext context, IUploadService upload, IHostingEnvironment hostingEnvironment,
        ...
        public IActionResult Index()
        {
            ...
            ViewData["CrumbAssetCancle"] = _sharedResources["Xuất hồ sơ"];
            return View();
        }
        ...
        {
            int intBegin = (jTablePara.CurrentPage - 1) * jTablePara.Length;
            var query = (from a in _context.RecordDeliveryDetails
                         join b in _context.RecordDeliveryHeaders on a.DeliveryCode equals b.DeliveryCode
                         let packs = _context.RecordsPacks.Where(x => x.PackCode.Equals(a.RecordCode)).OrderByDescending(x => x.ID)
        ...
        public JsonResult InsertRecordDeliveryDetail([FromBody] ModelDetail obj)
        {
            ...
            try
            {
                ...
                {
                    msg.Error = true;
                    msg.Title = "Vui lòng chọn hồ sơ xuất";
                    return Json(msg);
                }

                var listDetail = new List<RecordDeliveryDetail>();

                var packs = _context.RecordsPacks.Where(x => !x.IsDeleted).ToList();
                ...
                foreach(var item in obj.ListPack.Where(x => !string.IsNullOrEmpty(x.PackCode)))
                {
                    ...
                    {
                        var detail = new RecordDeliveryDetail
                        {
                            DeliveryCode = obj.DeliveryCode,
                            RecordCode = item.PackCode
                        };
                        listDetail.Add(detail);
                        _context.RecordDeliveryDetails.Add(detail);

                        var pack = _context.RecordsPacks.FirstOrDefault(x => x.PackCode.Equals(item.PackCode) && !x.IsDeleted);
                        if (pack != null)
                        {
                            pack.IsDeleted = true;

                            _context.RecordsPacks.Update(pack);
                            ...
                            foreach(var child in childs)
                            {
                                ...
                                {
                                    var childDetail = new RecordDeliveryDetail
                                    {
                                        DeliveryCode = obj.DeliveryCode,
                                        RecordCode = child.Code
                                    };
                                    listDetail.Add(childDetail);
                                    _context.RecordDeliveryDetails.Add(childDetail);

                                    var childpack = _context.RecordsPacks.FirstOrDefault(x => x.PackCode.Equals(child.Code) && !x.IsDeleted);
                                    if(childpack != null)
                                    {
                ...
                msg.Title = "Xuất hồ sơ thành công";
            }
            catch (Exception ex)
            {
                msg.Error = false;
        ...
        public JsonResult DeleteDetail(int id)
        {
            ...
            try
            {
                var detail = _context.RecordDeliveryDetails.FirstOrDefault(x => x.ID == id);
                if (detail != null)
                {
                    var packs = _context.RecordsPacks.Where(x => !x.IsDeleted && x.PackCode.Equals(detail.RecordCode)).OrderByDescending(x => x.ID);
                    if (packs.Any())
                    {
                    ...
                    _context.RecordDeliveryDetails.Remove(detail);
                    _context.SaveChanges();
