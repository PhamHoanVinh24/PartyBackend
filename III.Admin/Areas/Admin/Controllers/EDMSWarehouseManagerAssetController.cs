...
{
    [Area("Admin")]
    public class EDMSWarehouseManagerAssetController : BaseController
    {
        private readonly EIMDBContext _context;
        private readonly IHostingEnvironment _hostingEnvironment;
        private readonly IStringLocalizer<EDMSWarehouseManagerDocumentController> _stringLocalizer;
        private readonly IStringLocalizer<EDMSDiagramWarehouseDocumentController> _stringLocalizerDw;
        private readonly IStringLocalizer<SharedResources> _sharedResources;

        public EDMSWarehouseManagerAssetController(EIMDBContext context, IHostingEnvironment hostingEnvironment,
            IStringLocalizer<EDMSWarehouseManagerDocumentController> stringLocalizer,
            IStringLocalizer<SharedResources> sharedResources,
            IStringLocalizer<EDMSDiagramWarehouseDocumentController> stringLocalizerDw)
        {
            _context = context;
            _hostingEnvironment = hostingEnvironment;
            _stringLocalizer = stringLocalizer;
            _stringLocalizerDw = stringLocalizerDw;
            _sharedResources = sharedResources;
        }
        [Breadcrumb("ViewData.CrumbEdmsWHManageDoc", AreaName = "Admin", FromAction = "Index", FromController = typeof(MenuAssetOperationController))]
        ...
        public object JTable([FromBody]JTableModelExtend jTablePara)
        {
            ...
            var query = from a in _context.EDMSWareHouseAssets
        ...
        {
            int intBeginFor = (jTablePara.CurrentPage - 1) * jTablePara.Length;
            var query = from a in _context.EDMSFloorAssets
        ...
        public object JTableLine([FromBody]JTableLineModel jTablePara)
        {
            ...
            var query = from a in _context.EDMSLineAssets
        ...
        public object JTableRack([FromBody]JTableRackModel jTablePara)
        {
            ...
            var query = from a in _context.EDMSRackAssets
        ...
        public object GetListWareHouse()
        {
            try
            {
                var rs = _context.EDMSWareHouseAssets.Where(x => x.WHS_Flag != true).ToList();
                return Json(rs);
            }
            catch (Exception ex)
            {
        ...
        public object GetListFloor()
        {
            try
            {
                var rs = _context.EDMSFloorAssets.ToList();
                return Json(rs);
            }
            catch (Exception ex)
            {
        ...
        public object GetListFloorByWareHouseCode(string wareHouseCode)
        {
            try
            {
                var rs = _context.EDMSFloorAssets.Where(x => x.WHS_Code.Equals(wareHouseCode)).ToList();
                return Json(rs);
            }
            catch (Exception ex)
            {
        ...
        public object GetListLine(string storeCode)
        {
            try
            {
                if (string.IsNullOrEmpty(storeCode))
                {
                    ...
                    var rs = _context.EDMSLineAssets.ToList();
                    return Json(rs);
                }
                else
                {
                    var rs = _context.EDMSLineAssets.Where(x => x.LineCode.IndexOf(storeCode) > 0).ToList();
                    return Json(rs);
                }
            }
            catch (Exception ex)
        ...
        public object GetListRack()
        {
            try
            {
                var rs = _context.EDMSRackAssets.ToList();
                return Json(rs);
            }
            catch (Exception ex)
            {
        ...
        public object GetListRackByLineCode(string lineCode)
        {
            try
            {
                var rs = _context.EDMSRackAssets.Where(x => x.LineCode.Equals(lineCode)).ToList();
                return Json(rs);
            }
            catch (Exception ex)
            {
        ...
        public object GetDetailWareHouse(string wareHouseCode)
        {
            try
            {
                ...
                var rs = _context.EDMSWareHouseAssets.FirstOrDefault(x => x.WHS_Code == wareHouseCode && !x.WHS_Flag);
                if (rs != null)
                {
                    var listFloor = _context.EDMSFloorAssets.Where(x => x.WHS_Code.Equals(rs.WHS_Code)).ToList();
                    if (listFloor.Count > 0)
                    {
                        ...
                        foreach (var item in listFloor)
                        {
                            var listLine = _context.EDMSLineAssets.Where(x => x.FloorCode.Equals(item.FloorCode)).ToList();
                            if (listLine.Count > 0)
                            {
                                ...
                                {
                                    var listRack = _context.EDMSRackAssets.Where(x => x.LineCode.Equals(line.LineCode)).ToList();
                                    if (listRack.Count > 0)
                                    {
        ...
        public object GetBoxPosition(string boxCode)
        {
            try
            {
                ...
                if (rs != null)
                {
                    ...
                    if (rs.RackCode != null)
                        RackName = _context.EDMSRackAssets.FirstOrDefault(x => x.RackCode == rs.RackCode)?.RackName;
                    if (rs.LineCode != null)
                        LineName = _context.EDMSLineAssets.FirstOrDefault(x => x.LineCode == rs.LineCode)?.L_Text;
                    if (rs.FloorCode != null)
                        FloorName = _context.EDMSFloorAssets.FirstOrDefault(x => x.FloorCode == rs.FloorCode)?.FloorName;
                    if (rs.WHS_Code != null)
                        WareHouseName = _context.EDMSWareHouseAssets.Where(x => x.WHS_Flag != true).FirstOrDefault(x => x.WHS_Code == rs.WHS_Code)?.WHS_Name;
        ...
        public string GetObjTypePosition(string objCode, string type)
        {
            try
            {
                ...
                switch (type)
                {
                    ...
                    case "OBJ_RACK":
                        var obj = _context.EDMSRackAssets.FirstOrDefault(x => x.RackCode == objCode);
                        if (obj != null)
                        {
                            RackName = obj.RackName;
                            var objLine = _context.EDMSLineAssets.FirstOrDefault(x => x.LineCode == obj.LineCode);
                            if (objLine != null)
                            {
                                LineName = objLine.L_Text;
                                var objFloor = _context.EDMSFloorAssets.FirstOrDefault(x => x.FloorCode == objLine.FloorCode);
                                if (objFloor != null)
                                {
        ...
        //Thêm sửa xóa tầng
        [HttpPost]
        public JsonResult InsertFloor(EDMSFloorAsset obj, IFormFile imageFloor, IFormFile mapDesgin)
        {
            var msg = new JMessage();
            try
            {
                var floor = _context.EDMSFloorAssets.FirstOrDefault(x => x.FloorCode.Equals(obj.FloorCode));
                if (floor == null)
                {
                    var maxFloor = _context.EDMSWareHouseAssets.FirstOrDefault(x => x.WHS_Flag != true && x.WHS_Code.Equals(obj.WHS_Code)).WHS_CNT_Floor;
                    var countFloor = _context.EDMSFloorAssets.Where(x => x.WHS_Code.Equals(obj.WHS_Code)).Count();
                    if (countFloor < maxFloor)
                    {
                        var floorObj = new EDMSFloorAsset
                        {
                        ...
                        _context.EDMSFloorAssets.Add(floorObj);

                        //Thêm vào bảng quản lý QR_CODE
                        var qrCode = new EDMSWhsQrCodeAsset
                        {
                        ...
                        _context.EDMSWhsQrCodeAssets.Add(qrCode);
        ...
        public JsonResult UpdateFloor(EDMSFloor obj, IFormFile imageFloor, IFormFile mapDesgin)
        {
            ...
            try
            {
                ...
                if (floor != null)
                {
                    ...
                    var cnt = _context.EDMSLineAssets.Where(x => x.FloorCode == floor.FloorCode).Count();
                    if (cnt > obj.CNT_Line)
                    {
                        msg.Error = true;
                    ...
                    else
                    {
                        ...
                        _context.EDMSFloorAssets.Update(floor);

                        var checkExits = _context.EDMSWhsQrCodeAssets.FirstOrDefault(x => x.OBJ_Code == obj.FloorCode && x.OBJ_Type == "OBJ_FLOOR");
                        if (checkExits == null)
                        {
                            //Thêm vào bảng quản lý QR_CODE
                            var qrCode = new EDMSWhsQrCodeAsset
        ...
        public JsonResult DeleteFloor(int floorId)
        {
            ...
            try
            {
                ...
                if (count == 0)
                {
                    var floor = _context.EDMSFloorAssets.FirstOrDefault(x => x.Id.Equals(floorId));
                    if (floor != null)
                    {
                        //Check xem có tồn tại dãy không
                        var chkExistLine = _context.EDMSLineAssets.Any(x => x.FloorCode == floor.FloorCode);
                        if (chkExistLine)
                        {
                            msg.Error = true;
                            msg.Title = _stringLocalizerDw["EDMSWM_VALIDATE_DEL_ROW_BEFORE"];
                            return Json(msg);
                        }

                        _context.EDMSFloorAssets.Remove(floor);
                        ...
                        var listCheck = _context.EDMSWhsQrCodeAssets.Where(x => x.OBJ_Code == floor.FloorCode && x.OBJ_Type == "OBJ_FLOOR");
                        if (listCheck.Any())
                        {
                            _context.EDMSWhsQrCodeAssets.RemoveRange(listCheck);
                        }
        ...
        //Thêm sửa xóa dãy
        [HttpPost]
        public JsonResult InsertLine([FromBody]EDMSLineAsset obj)
        {
            var msg = new JMessage();
            try
            {
                var line = _context.EDMSLineAssets.FirstOrDefault(x => x.LineCode.Equals(obj.LineCode));

                if (line == null)
                {
                    var floor = _context.EDMSFloorAssets.FirstOrDefault(x => x.FloorCode == obj.FloorCode);
                    var countLineInFloor = _context.EDMSLineAssets.Where(x => x.FloorCode == obj.FloorCode).Count();
                    if (floor.CNT_Line > countLineInFloor)
                    {
                        var lineObj = new EDMSLineAsset
                        {
                            FloorCode = obj.FloorCode,
                        ...
                        _context.EDMSLineAssets.Add(lineObj);
        ...
        [HttpPost]
        public JsonResult UpdateLine([FromBody]EDMSLineAsset obj)
        {
            var msg = new JMessage();
            try
            {
                var line = _context.EDMSLineAssets.FirstOrDefault(x => x.Id.Equals(obj.Id));
                if (line != null)
                {
                    //check số rack phải >= rack đã khai báo
                    var cnt = _context.EDMSRackAssets.Where(x => x.LineCode == line.LineCode).Count();
                    if (cnt > obj.CNT_Rack)
                    {
                        msg.Error = true;
                    ...
                    else
                    {
                        ...
                        var checkExits = _context.EDMSWhsQrCodeAssets.FirstOrDefault(x => x.OBJ_Code == obj.LineCode && x.OBJ_Type == "OBJ_LINE");
                        if (checkExits == null)
                        {
                            //Thêm vào bảng quản lý QR_CODE
                            var qrCode = new EDMSWhsQrCodeAsset
        ...
        public JsonResult GetLineInfo(int lineId)
        {
            ...
            try
            {
                var line = _context.EDMSLineAssets.FirstOrDefault(x => x.Id.Equals(lineId));
                if (line != null)
                {
                    msg.Object = line;
                }
        ...
        public JsonResult DeleteLine(int lineId)
        {
            ...
            try
            {
                ...
                if (line != null)
                {
                    ...
                    var chkExist = _context.EDMSRackAssets.Any(x => x.LineCode == line.LineCode);
                    if (chkExist)
                    {
                        msg.Error = true;
                        msg.Title = _stringLocalizerDw["EDMSWM_VALIDATE_DEL_SHELF_BEFORE"];
                        return Json(msg);
                    }
                    ...
                    var listCheck = _context.EDMSWhsQrCodeAssets.Where(x => x.OBJ_Code == line.LineCode && x.OBJ_Type == "OBJ_LINE");
                    if (listCheck.Any())
                    {
                        _context.EDMSWhsQrCodeAssets.RemoveRange(listCheck);
                    }

                    _context.EDMSLineAssets.Remove(line);
                    _context.SaveChanges();
                    msg.Error = false;
        ...
        //Thêm sửa xóa kệ
        [HttpPost]
        public JsonResult InsertRack([FromBody]EDMSRackAsset obj)
        {
            var msg = new JMessage();
            try
            {
                var rack = _context.EDMSRackAssets.FirstOrDefault(x => x.RackCode.Equals(obj.RackCode));
                if (rack == null)
                {

                    var line = _context.EDMSLineAssets.FirstOrDefault(x => x.LineCode == obj.LineCode);
                    var countRackInLine = _context.EDMSRackAssets.Where(x => x.LineCode == obj.LineCode).Count();

                    if (line.CNT_Rack > countRackInLine)
                    {
                        //add Rack
                        obj.QR_Code = obj.RackCode;
                        _context.EDMSRackAssets.Add(obj);
        ...
        public JsonResult GetRackInfo(int rackId)
        {
            ...
            try
            {
                var rack = _context.EDMSRackAssets.FirstOrDefault(x => x.Id.Equals(rackId));
                if (rack != null)
                {
                    msg.Object = rack;
                }
        ...
        [HttpPost]
        public JsonResult UpdateRack([FromBody]EDMSRackAsset obj)
        {
            var msg = new JMessage();
            try
            {
                var rack = _context.EDMSRackAssets.FirstOrDefault(x => x.Id.Equals(obj.Id));
                ...
                if (rack != null)
                {
                    ...
                    _context.EDMSRackAssets.Update(rack);

                    //remove position old
                    var listRackOld = _context.EDMSWhsQrCodeAssets.Where(x => x.OBJ_Code == obj.RackCode && x.OBJ_Type == "OBJ_RACK");
                    if (listRackOld.Any())
                    {
                        _context.EDMSWhsQrCodeAssets.RemoveRange(listRackOld);
                    }
        ...
        public JsonResult DeleteRack(int rackId)
        {
            ...
            try
            {
                ...
                if (rack != null)
                {
                    ...
                    var whsCode = (from a in _context.EDMSRackAssets.Where(x => x.Id.Equals(rackId))
                                   ...
                                   join d in _context.EDMSWareHouseAssets.Where(x => x.WHS_Flag != true) on c.WHS_Code equals d.WHS_Code
                                   group d.WHS_Code by d.WHS_Code into g
                                   select g.Key).FirstOrDefault();
                    var countRack = (from b in _context.EDMSFloorAssets.Where(x => x.WHS_Code == whsCode)
                                     join c in _context.EDMSLineAssets on b.FloorCode equals c.FloorCode
                                     join d in _context.EDMSRackAssets.Where(x => !x.Id.Equals(rackId)) on c.LineCode equals d.LineCode
                                     select d.Id).Count();
                    //if (countRack == 0)
                    //{
                    ...
                    //remove rack
                    _context.EDMSRackAssets.Remove(rack);

                    var listMappingDelete = _context.EDMSFilePackCoverAssets.Where(x => x.RackCode.Equals(rack.RackCode)).ToList();
                    if (listMappingDelete.Count > 0)
                    {
                        ...
                        _context.EDMSFilePackCoverAssets.UpdateRange(listMappingDelete);
                    }

                    //remove position old
                    var listPositionOld = _context.EDMSWhsQrCodeAssets.Where(x => x.OBJ_Code == rack.RackCode && x.OBJ_Type == "OBJ_RACK_POSITION");
                    if (listPositionOld.Any())
                    {
                        _context.EDMSWhsQrCodeAssets.RemoveRange(listPositionOld);
                    }
                    _context.SaveChanges();
                    msg.Error = false;
        ...
        {
            decimal count = 0;
            var query = from a in _context.EDMSRackAssets.Where(x => x.Id == rackId)
        ...
        public IActionResult Translation(string lang)
        {
            ...
            var query = _stringLocalizer.GetAllStrings().Select(x => new { x.Name, x.Value })
                .Union(_stringLocalizerDw.GetAllStrings().Select(x => new { x.Name, x.Value }))
