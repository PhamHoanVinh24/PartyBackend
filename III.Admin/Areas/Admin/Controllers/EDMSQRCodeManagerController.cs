...
namespace III.Admin.Controllers
{
    ...
    public class EDMSQRCodeManagerController : BaseController
    {
        ...
        public EDMSQRCodeManagerController(EIMDBContext context, IStringLocalizer<EDMSQRCodeManagerController> stringLocalizer, IStringLocalizer<SharedResources> sharedResources)
        {
            _context = context;
        ...
        [Breadcrumb("ViewData.CrumbEdmsQrCodeManage", AreaName = "Admin", FromAction = "Index", FromController = typeof(WarehouseDigitalHomeController))]
        ...
        public object GetListWareHouse()
        {
            try
            {
                var rs = _context.EDMSWareHouseDocuments.Where(x => x.WHS_Flag != true).Select(x => new
                {
                    Code = x.WHS_Code,
                    Name = x.WHS_Name,
                    TypeName = "Kho",
        ...
        [HttpPost]
        public object GetListCabinet()
        {
            try
            {
                var rs = _context.ObjectiverPackCovers.Where(x => !x.IsDeleted)
                                                      .Select(x => new
                                                      {
                                                          Code = x.ObjPackCode,
                                                          Name = x.Name
