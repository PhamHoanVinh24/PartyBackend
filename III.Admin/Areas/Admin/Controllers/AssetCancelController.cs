...
namespace III.Admin.Controllers
{
    ...
    public class AssetCancelController : BaseController
    {
        ...
        public IActionResult Index()
        {
            ...
            ViewData["CrumbAssetCancle"] = _sharedResources["COM_CRUMB_ASSET_CANCLE"];
            return View();
        }
        ...
        public object JTable([FromBody]JTableModelAct jTablePara)
        {
            ...
            var query = (from a in _context.AssetCancelHeaders
                         ...
                         join c in _context.AdOrganizations on a.Branch equals c.OrgAddonCode into c1
                        ...
                        && (toDate == null || (a.CancelTime >= fromDate))
                         select new
                         {
                             ...
                             a.AssetID,
                             a.Title,
                             Branch = c2 != null ? c2.OrgName : "Không xác định",
                             Person = d2.fullname,
                             a.Note,
                             Status = a.Status,
                             a.CancelTime
        ...
        {
            var project = _context.AssetCancelHeaders.FirstOrDefault(x => x.TicketCode.Equals(code) && !x.IsDeleted);
            return Json(project);
        }
        ...
        {
            List<ComboxModel> list = new List<ComboxModel>();
            var data = _context.AssetCancelHeaders.FirstOrDefault(x => x.TicketCode.Equals(code) && !x.IsDeleted);
            var check = _context.WorkflowInstances.FirstOrDefault(x => !x.IsDeleted.Value && x.ObjectInst.Equals(data.TicketCode)
                && x.ObjectType.Equals(EnumHelper<ObjectType>.GetDisplayValue(ObjectType.AssetCancel)));
        ...
        {
            var data = _context.AssetCancelHeaders.Where(x => !x.IsDeleted && x.TicketCode.Equals(code)).Select(x => new
            {
                x.Status
            });
            return data;
        ...
        public object GetItemTemp(int id)
        {
            ...
            try
            {
                ...
                var data = _context.AssetCancelHeaders.FirstOrDefault(x => x.AssetID == id && !x.IsDeleted);
                if (data == null)
                {
                    msg.Error = true;
                    msg.Title = _stringLocalizer["ASSET_CANCEL_MSG_TICKET_NO_EXIST"];
                    return Json(msg);
                }
        ...
        public JsonResult InsertAsset([FromBody]AssetCancelDetail obj)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    {
                        msg.Error = true;
                        msg.Title = String.Format(_sharedResources["COM_ERR_EXIST"], _stringLocalizer["ASSET_CANCEL_ASSET"]);
                        return Json(msg);
                    }
                }
            }
        ...
        public object JTableADD([FromBody]JTableModelAct jTablePara)
        {
            int intBegin = (jTablePara.CurrentPage - 1) * jTablePara.Length;
            var listCommon = _context.CommonSettings.Select(x => new { x.CodeSet, x.ValueSet });
            var query = (from a in _context.AssetCancelDetails.Where(x => !x.IsDeleted)
        ...
        public object JTableFile([FromBody]JTableModelFile jTablePara)
        {
            ...
            var fromDate = !string.IsNullOrEmpty(jTablePara.FromDate) ? DateTime.ParseExact(jTablePara.FromDate, "dd/MM/yyyy", CultureInfo.InvariantCulture) : (DateTime?)null;
            var toDate = !string.IsNullOrEmpty(jTablePara.ToDate) ? DateTime.ParseExact(jTablePara.ToDate, "dd/MM/yyyy", CultureInfo.InvariantCulture) : (DateTime?)null;
            var query = ((from a in _context.EDMSRepoCatFiles.Where(x => x.ObjectCode == jTablePara.AssetCode && x.ObjectType == EnumHelper<ObjectType>.GetDisplayValue(ObjectType.AssetCancel))
                ...
                if (Array.IndexOf(LuceneExtension.fileMimetypes, mimeType) >= 0 && (Array.IndexOf(LuceneExtension.fileExt, extension.ToUpper()) >= 0))
                {
                    ...
                    if (!obj.IsMore)
                    {
                        ...
                        var repoDefault = _context.EDMSRepoDefaultObjects.FirstOrDefault(x => !x.IsDeleted
                               && x.ObjectCode.Equals(obj.AssetCode) && x.ObjectType.Equals(EnumHelper<ObjectType>.GetDisplayValue(ObjectType.AssetCancel)));
                        if (repoDefault != null)
                        {
